...
export async function readApprovalNotes(cwd, runId) {
...
export async function addApprovalNote(cwd, runId, args) {
    ...
    const note = { ...unsigned, signature: localPayloadSignature(unsigned) };
...
export function renderApprovalNotes(notes) {
...
export async function verifyApprovalNotes(cwd, runId) {
    ...
    for (const note of notes) {
        ...
        const signatureOk = localPayloadSignature(unsigned).payloadHash !== note.signature.payloadHash ? false : true;
        const artifactsOk = (await Promise.all(note.artifactHashes.map(async (artifact) => {
