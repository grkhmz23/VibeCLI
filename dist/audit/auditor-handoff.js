...
export async function createAuditorHandoff(cwd, runId, options = {}) {
    ...
    const handoff = {
        ...
        executiveSummary: "Local auditor handoff bundle generated from redacted VibeCLI audit-support artifacts.",
        ...
        signedReports: {
            ...
            complianceBundleSigned: pathExists(join(runPath, "audit/compliance/COMPLIANCE_BUNDLE_ENVELOPE.json")),
            provenanceSigned: pathExists(join(runPath, "provenance/provenance-envelope.json")),
            orgPolicySigned: pathExists(join(cwd, ".vibecli/org/policy-bundles/latest/ORG_POLICY_ENVELOPE.json"))
        },
        verification: {
            ...
            complianceBundleVerified: (await verifyComplianceBundle(cwd, runId).catch(() => undefined))?.ok ?? null,
    ...
    await store.writeTextArtifact(runId, "audit/auditor-handoff/AUDITOR_HANDOFF.md", renderAuditorHandoff(handoff));
    const archive = {};
    for (const file of [
    ...
    const archivePath = join(runPath, "audit", "auditor-handoff", `vibecli-auditor-handoff-${runId}.tar.gz`);
...
export async function verifyAuditorHandoff(cwd, runId) {
    const store = new RunStore(cwd);
...
async function readEvidenceIndex(cwd, runId) {
    const path = join(new RunStore(cwd).runPath(runId), "audit", "AUDIT_EVIDENCE_MAP.json");
    const map = await readJson(path).catch(() => undefined);
    return map?.controls?.flatMap((control) => control.evidence ?? []) ?? [];
}
async function readOptionalJson(path) {
    return pathExists(path) ? await readJson(path).catch(() => ({})) : {};
}
function stateEvidenceLifecycle(state) {
    return {
...
function stateEvidenceDisposal(state) {
    return {
...
async function writeAuditorHandoffManifest(cwd, runId, archiveSha256) {
    const store = new RunStore(cwd);
    const root = join(store.runPath(runId), "audit", "auditor-handoff");
    const files = [];
...
function renderAuditorHandoff(handoff) {
