...
export async function generateAuditEvidenceMap(cwd, runId, options = {}) {
    ...
    const summary = {
        ...
        criticalMissing: controls.filter((control) => control.status === "missing" && control.severity === "critical").length,
        highMissing: controls.filter((control) => control.status === "missing" && control.severity === "high").length
    };
    const map = {
        runId,
...
Controls:
${map.controls
        .map((control) => `- ${control.id} (${control.severity}, ${control.status}): ${control.title}${control.gaps.length ? `\n  Gaps: ${control.gaps.join("; ")}` : ""}`)
        .join("\n")}
`;
}
