...
export async function runPackageInstallCheck(cwd) {
    const checks = [];
    ...
    try {
        ...
        if (file) {
            ...
            tarball.containsPrivateArtifacts = /\.private\.pem|\.vibecli\/keys|\.vibecli\/org\/keys/.test(text);
    ...
    if (tarball.created && tarball.path) {
        try {
            ...
            await execFileAsync(binPath, ["doctor"], { cwd: project, timeout: 20_000 }).catch(() => undefined);
    ...
    if (tarball.containsPrivateArtifacts ||
    ...
    await import("node:fs/promises").then((fs) => fs.writeFile(join(cwd, ".vibecli", "beta", "reports", "PACKAGE_INSTALL_CHECK.md"), `# Package Install Check\n\nStatus: ${report.status}\nErrors: ${errors.length}\nWarnings: ${warnings.length}\nNo package was published.\n`, "utf8"));
