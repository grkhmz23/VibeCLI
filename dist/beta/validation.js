...
export function validateBetaConfig(config) {
    const errors = [];
    for (const [name, path] of [
        ...
        if (!underDotVibeBeta(path))
            errors.push(`beta.${name} must be under .vibecli/beta`);
        if (resolve(path) === resolve("."))
            errors.push(`beta.${name} must not resolve to repo root`);
    }
