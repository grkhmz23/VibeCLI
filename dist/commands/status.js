...
async function printStatus(runId) {
    ...
    console.log(`Beta: verdict=${dogfood.latestBetaVerdict} dogfood=${dogfood.latestDogfoodRunId ?? "none"}`);
    const beta = await readBetaState(process.cwd());
    console.log(`Beta RC: verdict=${beta.latestBetaVerdict} blockers=${beta.blockers} warnings=${beta.warnings} accepted=${beta.acceptedWarnings}`);
}
async function watchStatus(runId) {
