...
export async function runDocsCheck(cwd, options = {}) {
    ...
    const missingCommands = commands.filter((command) => !docs.includes(`vibe ${command}`) && !docs.includes(`/${command}`));
    const missingConfirmations = confirmationCommands.filter((confirmation) => !docs.includes(confirmation));
    ...
    await import("node:fs/promises").then((fs) => fs.writeFile(join(cwd, ".vibecli", "dogfood", "reports", "DOCS_CHECK.md"), `# Docs Check\n\nBlockers: ${report.blockers.length}\nWarnings: ${report.warnings.length}\n`, "utf8"));
    ...
    if (options.strict) {
        ...
        await import("node:fs/promises").then((fs) => fs.writeFile(join(cwd, ".vibecli", "beta", "reports", "DOCS_STRICT_CHECK.md"), `# Docs Strict Check\n\nStatus: ${report.blockers.length ? "failed" : "passed"}\nBlockers: ${report.blockers.length}\nWarnings: ${report.warnings.length}\n`, "utf8"));
        await import("../beta/config.js").then(({ updateBetaState }) => updateBetaState(cwd, { latestReports: { docsStrictCheck: strictPath } }));
    }
    return report;
}
//# sourceMappingURL=docs-coverage.js.map