...
async function timed(command, fn) {
...
export async function createDogfoodPlan(cwd, options = {}) {
    const config = await loadConfig(cwd);
    if (options.writeFixtures)
        await createDogfoodFixtures(cwd);
    return {
...
export async function runDogfood(cwd, options = {}) {
    const config = await loadConfig(cwd);
    if (options.applyFixturePatches && options.confirm !== "APPLY DOGFOOD FIXTURE PATCHES") {
        throw new Error('Applying dogfood fixture patches requires exact confirmation: "APPLY DOGFOOD FIXTURE PATCHES"');
    }
    ...
    for (const fixture of fixtureSet.fixtures) {
        ...
        const record = async (name, fn) => {
            ...
            await appendFile(logPath, `${JSON.stringify({ fixture: fixture.type, ...entry.record })}\n`, "utf8");
        ...
        if (vibeRunId) {
            ...
            workspaceCreated = Boolean(await record("vibe workspace", () => buildReviewWorkspace(fixture.path, vibeRunId)));
            reviewCreated = Boolean(await record("vibe review", () => reviewRun(fixture.path, vibeRunId)));
            await record("vibe diff --check", () => checkRunDiffs(fixture.path, vibeRunId));
            const ledger = await record("vibe ledger --verify", () => verifyLedger(fixture.path, vibeRunId));
            ...
            if (options.applyFixturePatches && config.dogfood.allow_fixture_patch_apply) {
                await record("vibe apply", () => applyRun(fixture.path, vibeRunId, { confirm: `APPLY ${vibeRunId}` }));
            }
        }
    ...
    const report = {
        ...
        blockers: fixtures.flatMap((fixture) => fixture.errors.map((error) => `${fixture.fixture}: ${error}`)),
