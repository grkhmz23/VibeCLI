...
const secretLike = /(sk-[A-Za-z0-9_-]{20,}|gh[pousr]_[A-Za-z0-9_]{20,}|Bearer\s+[A-Za-z0-9._-]{20,}|-----BEGIN [A-Z ]*PRIVATE KEY-----|[a-z]+:\/\/[^/\s:]+:[^@\s]+@|[A-Za-z0-9_=-]{40,})/i;
function underDotVibeDogfood(path) {
    const normalized = path.replaceAll("\\", "/");
    return normalized === ".vibecli/dogfood" || normalized.startsWith(".vibecli/dogfood/");
}
function scanSecrets(value, path = "dogfood") {
...
export function validateDogfoodConfig(config) {
    const errors = [];
    for (const [name, path] of [
        ...
        if (!underDotVibeDogfood(path))
            errors.push(`dogfood.${name} must be under .vibecli/dogfood`);
        if (resolve(path) === resolve("."))
            errors.push(`dogfood.${name} must not resolve to repo root`);
    }
    for (const fixture of config.default_matrix) {
        if (!dogfoodFixtureTypes.includes(fixture)) {
            errors.push(`dogfood.default_matrix contains unknown fixture ${fixture}`);
        }
    }
    if (config.allow_live_provider_smoke) {
        errors.push("dogfood.allow_live_provider_smoke must default false unless explicitly changed for local use");
    }
