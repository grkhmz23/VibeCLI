...
export async function getDisposalApprovals(cwd, runId) {
    const path = approvalPath(cwd, runId);
    if (pathExists(path))
        return readJson(path);
    return writeDisposalApprovals(cwd, runId, []);
}
export async function addDisposalApproval(cwd, runId, options) {
    if (options.confirm !== `ADD DISPOSAL APPROVAL ${runId}`) {
        throw new Error(`Disposal approval requires exact confirmation: ADD DISPOSAL APPROVAL ${runId}`);
    }
    ...
    const planSha256 = (await sha256File(join(runPath, "evidence-lifecycle", "disposal", "DISPOSAL_PLAN.json"))).sha256;
    const candidateSha256 = (await sha256File(join(runPath, "evidence-lifecycle", "disposal", "DISPOSAL_CANDIDATES.json"))).sha256;
...
export async function verifyDisposalApprovals(cwd, runId) {
    const approvals = await getDisposalApprovals(cwd, runId);
    const runPath = new RunStore(cwd).runPath(runId);
    const planSha256 = pathExists(join(runPath, "evidence-lifecycle", "disposal", "DISPOSAL_PLAN.json"))
        ? (await sha256File(join(runPath, "evidence-lifecycle", "disposal", "DISPOSAL_PLAN.json")))
            .sha256
        : "";
    const candidateSha256 = pathExists(join(runPath, "evidence-lifecycle", "disposal", "DISPOSAL_CANDIDATES.json"))
        ? (await sha256File(join(runPath, "evidence-lifecycle", "disposal", "DISPOSAL_CANDIDATES.json"))).sha256
        : "";
    const ok = approvals.approvals.every((approval) => approval.planSha256 === planSha256 && approval.candidateSha256 === candidateSha256);
    return { ok, message: ok ? "approvals verified" : "approval artifact hash mismatch", approvals };
}
async function writeDisposalApprovals(cwd, runId, approvals) {
    const config = await loadConfig(cwd);
    ...
    if (approved.length < policy.min_approvals)
        blockingReasons.push("Minimum approvals not met.");
    if (policy.require_distinct_reviewers &&
    ...
    await store.writeTextArtifact(runId, "evidence-lifecycle/disposal/DISPOSAL_APPROVALS.md", renderApprovals(result));
    await updateEvidenceDisposalState(store, runId, (state) => {
        state.approvals = {
            status: result.quorum.status === "met"
                ? "met"
...
function approvalPath(cwd, runId) {
    return join(new RunStore(cwd).runPath(runId), "evidence-lifecycle", "disposal", "DISPOSAL_APPROVALS.json");
}
