...
export function classifyDisposalPath(path) {
    const normalized = path.replace(/\\/g, "/");
    const name = basename(normalized);
    if (normalized.includes("/.git/") || normalized.startsWith(".git/"))
        return "git";
    if (normalized.endsWith(".private.pem") || /private[-_]?key/i.test(name))
        return "private-keys";
    if (/^\.env($|\.)|\/\.env($|\.)/.test(normalized) && !normalized.endsWith(".env.example")) {
        return "env-files";
    }
    if (normalized.includes("org/keys/"))
        return "org-keys";
    if (normalized.includes(".vibecli/keys/") || normalized.includes("provenance-key")) {
        return "provenance-keys";
    }
    if (/\.tar\.gz$|\.zip$|archive/i.test(normalized))
        return "archives";
    if (/^(src|lib|app|packages|docs)\//.test(normalized) || /package(-lock)?\.json$/.test(name)) {
        return "source-code";
    }
    if (normalized.startsWith("ledger-manifest.json"))
        return "run-ledger";
    if (normalized.startsWith("evidence-lifecycle/disposal/"))
        return "disposal";
    if (normalized.startsWith("evidence-lifecycle/"))
...
export function isProtectedDisposalPath(path) {
    const normalized = path.replace(/\\/g, "/");
...
export function conservativeCandidateReason(path) {
    const normalized = path.replace(/\\/g, "/");
    if (isProtectedDisposalPath(normalized))
        return null;
    ...
    if (normalized === "agent-events.jsonl")
        return "Archived event log can be locally disposed.";
    ...
    if (normalized === "routing-plan.json")
        return "Archived routing plan can be locally disposed.";
    return null;
}
