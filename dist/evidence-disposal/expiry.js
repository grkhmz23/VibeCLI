...
export async function evaluateDisposalEligibility(cwd, runId, options = {}) {
    ...
    const retention = pathExists(retentionPath)
        ? await readJson(retentionPath).catch(() => emptyRetention)
        : pathExists(planPath)
            ? await readJson(planPath).catch(() => emptyRetention)
            : emptyRetention;
    ...
    const result = {
        ...
        nextActions: blockingReasons.length === 0
    ...
    await store.writeTextArtifact(runId, "evidence-lifecycle/disposal/DISPOSAL_ELIGIBILITY.md", renderEligibility(result));
...
async function readApprovalStatus(cwd, runId) {
    const path = join(new RunStore(cwd).runPath(runId), "evidence-lifecycle", "disposal", "DISPOSAL_APPROVALS.json");
    if (!pathExists(path))
        return "missing";
    const approvals = await readJson(path).catch(() => undefined);
