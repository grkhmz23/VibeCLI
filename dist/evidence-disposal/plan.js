...
export async function createDisposalPlan(cwd, runId, options = {}) {
    ...
    const attestationPath = join(runPath, "evidence-lifecycle", "disposal", "DISPOSAL_ATTESTATION.json");
    const attestation = pathExists(attestationPath)
        ? await readJson(attestationPath).catch(() => undefined)
        : undefined;
    const requirements = [
        {
        ...
        },
        {
            ...
            message: eligibility.organizationApproval.status === "met"
    ...
    const gatesPass = requirements.every((requirement) => !requirement.required || requirement.satisfied);
    ...
    const status = gatesPass && eligibility.eligible
    ...
    await store.writeTextArtifact(runId, "evidence-lifecycle/disposal/DISPOSAL_PLAN.md", renderPlan(plan));
    const planHash = await sha256File(join(runPath, "evidence-lifecycle", "disposal", "DISPOSAL_PLAN.json")).catch(() => undefined);
...
async function readOrBuildCandidates(cwd, runId) {
    const path = join(new RunStore(cwd).runPath(runId), "evidence-lifecycle", "disposal", "DISPOSAL_CANDIDATES.json");
