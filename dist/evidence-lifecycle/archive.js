...
export async function previewEvidenceArchive(cwd, runId, options = {}) {
    ...
    return {
        runId,
        mode,
        files: inventory.files.filter((file) => includeInMode(file.includedInDefaultArchive, mode) && !file.excluded).length,
        excluded: inventory.files.filter((file) => file.excluded).length
    };
}
export async function createEvidenceArchive(cwd, runId, options = {}) {
    if (options.confirm !== `ARCHIVE EVIDENCE ${runId}`) {
        throw new Error(`Evidence archive creation requires exact confirmation: ARCHIVE EVIDENCE ${runId}`);
    }
    const config = await loadEvidenceLifecycleConfig(cwd);
    if (config.require_ledger_pass_for_archive && !options.allowLedgerWarning) {
        ...
        if (!ledger?.ok)
            throw new Error("Ledger must pass before evidence archive creation.");
    }
    ...
    for (const file of inventory.files) {
        ...
        if (!pathExists(fullPath))
            continue;
        archiveContent[file.path] = redactEvidenceText(await readFile(fullPath, "utf8").catch(() => ""), 20_000);
        files.push({
            path: file.path,
            sha256: file.sha256,
    ...
    await writeFile(join(archiveDir, "EVIDENCE_ARCHIVE_MANIFEST.json"), `${JSON.stringify(manifest, null, 2)}\n`, "utf8");
    ...
    await store.writeArtifact(runId, "evidence-lifecycle/EVIDENCE_ARCHIVE_POINTER.json", {
        runId,
        archiveManifestPath: manifest.archivePath.replace(archiveName, "EVIDENCE_ARCHIVE_MANIFEST.json"),
...
export async function readArchiveManifest(cwd, runId) {
    const paths = await evidenceLifecyclePaths(cwd);
    return readJson(join(paths.archiveDir, runId, "EVIDENCE_ARCHIVE_MANIFEST.json"));
}
...
function includeInMode(included, mode) {
    if (!included)
        return false;
