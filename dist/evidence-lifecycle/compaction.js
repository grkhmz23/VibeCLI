...
export async function createCompactionReport(cwd, runId) {
    const inventory = await readOrCreateInventory(cwd, runId);
    const totalBytes = inventory.summary.totalBytes;
    const summaryBundleBytesEstimate = Math.min(totalBytes, Math.max(1024, inventory.summary.includedFiles * 512));
    ...
    const report = {
        ...
        estimatedSavingsPercent: totalBytes === 0 ? 0 : Math.round((estimatedSavingsBytes / totalBytes) * 10000) / 100,
    ...
    await store.writeTextArtifact(runId, "evidence-lifecycle/EVIDENCE_COMPACTION_REPORT.md", renderReport(report));
        ...
        artifactHashes: [
            {
                ...
                sha256: (await sha256File(join(new RunStore(cwd).runPath(runId), "evidence-lifecycle", "EVIDENCE_COMPACTION_REPORT.json"))).sha256
            }
        ]
    }).catch(() => undefined);
    return report;
}
export async function createCompactEvidenceBundle(cwd, runId, options = {}) {
    if (options.confirm !== `CREATE COMPACT EVIDENCE ${runId}`) {
        throw new Error(`Compact evidence bundle requires exact confirmation: CREATE COMPACT EVIDENCE ${runId}`);
    }
    ...
    for (const file of (await readOrCreateInventory(cwd, runId)).files) {
        if (file.excluded)
            excludedRawArtifacts.push(file.path);
    }
    ...
    await store.writeArtifact(runId, "evidence-lifecycle/compact/COMPACT_EVIDENCE_BUNDLE.json", bundle);
    await store.writeTextArtifact(runId, "evidence-lifecycle/compact/COMPACT_EVIDENCE_BUNDLE.md", renderBundle(bundle));
    const manifest = {
        runId,
        createdAt: new Date().toISOString(),
        algorithm: "sha256",
        ...
        files: [
            {
                ...
                ...(await sha256File(join(runPath, "evidence-lifecycle", "compact", "COMPACT_EVIDENCE_BUNDLE.json")))
            },
            {
                path: "COMPACT_EVIDENCE_BUNDLE.md",
                ...(await sha256File(join(runPath, "evidence-lifecycle", "compact", "COMPACT_EVIDENCE_BUNDLE.md")))
            }
        ],
        manifestHash: ""
    };
    manifest.manifestHash = sha256Text(JSON.stringify({ ...manifest, manifestHash: "" }, null, 2));
    await store.writeArtifact(runId, "evidence-lifecycle/compact/COMPACT_EVIDENCE_MANIFEST.json", manifest);
    await updateEvidenceLifecycleState(store, runId, (state) => {
...
export async function verifyCompactEvidenceBundle(cwd, runId) {
    const store = new RunStore(cwd);
    const runPath = store.runPath(runId);
    const manifestPath = join(runPath, "evidence-lifecycle", "compact", "COMPACT_EVIDENCE_MANIFEST.json");
...
async function readOrCreateInventory(cwd, runId) {
    const path = join(new RunStore(cwd).runPath(runId), "evidence-lifecycle", "EVIDENCE_INVENTORY.json");
