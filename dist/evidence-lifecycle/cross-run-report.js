...
export async function createEvidenceReport(cwd, options = {}) {
    ...
    const runs = await Promise.all(entries
        ...
        const retention = await readOptional(join(runPath, "org", "RETENTION_PLAN.json"));
        return {
            runId,
    ...
    const report = {
        ...
        summary: {
            ...
            archivedRuns: filtered.filter((run) => run.archiveStatus === "archived" || run.archiveStatus === "verified").length,
            legalHoldRuns: filtered.filter((run) => run.legalHold).length,
            missingInventory: filtered.filter((run) => run.warnings.includes("Missing evidence inventory.")).length,
    ...
    await import("node:fs/promises").then((fs) => fs.writeFile(join(dir, "EVIDENCE_REPORT.md"), renderReport(report), "utf8"));
    return report;
}
...
//# sourceMappingURL=cross-run-report.js.map