...
export async function legalHoldStatus(cwd, runId) {
    const path = legalHoldPath(cwd, runId);
    return pathExists(path) ? readJson(path) : { runId, status: "not_started" };
}
export async function enableLegalHold(cwd, runId, options) {
...
export async function releaseLegalHold(cwd, runId, options) {
...
function legalHoldPath(cwd, runId) {
    return join(new RunStore(cwd).runPath(runId), "evidence-lifecycle", "LEGAL_HOLD.json");
}
async function writeLegalHold(cwd, runId, record) {
...
async function recordLegalHoldEvent(cwd, runId, eventType, summary) {
    const path = legalHoldPath(cwd, runId);
...
function renderLegalHold(record) {
