...
export async function appendRetentionLedgerEvent(cwd, event) {
    ...
    const entry = {
        ...
        eventHash,
        previousChainHash,
        chainHash: sha256Text(JSON.stringify({ index: chain.length, runId: event.runId, eventHash, previousChainHash }))
    };
...
export async function readRetentionLedger(cwd) {
    const paths = await evidenceLifecyclePaths(cwd);
    const logPath = join(paths.retentionLedgerDir, "retention-ledger.jsonl");
    ...
    const chain = pathExists(join(paths.retentionLedgerDir, "retention-chain.json"))
        ? await readJson(join(paths.retentionLedgerDir, "retention-chain.json")).catch(() => [])
        : [];
    return { events, chain };
}
export async function retentionLedgerSummary(cwd, runId) {
...
export async function recordManualRetentionEvent(cwd, runId, options) {
    if (options.confirm !== `RECORD RETENTION EVENT ${runId}`) {
        throw new Error(`Retention ledger record requires exact confirmation: RECORD RETENTION EVENT ${runId}`);
    }
