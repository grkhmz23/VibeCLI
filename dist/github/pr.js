...
export async function githubPr(cwd, runId, options = {}) {
    ...
    if (options.push) {
        const dirty = await execFileAsync("git", ["status", "--short"], { cwd }).then(({ stdout }) => stdout.trim());
        if (dirty && !options.allowDirty)
            throw new Error("Refusing push with uncommitted changes.");
        if (!doctor.branch)
            throw new Error("Cannot push without a current branch.");
    ...
    state.lifecycle = {
        ...
        pr: {
            status: result.mode === "created" ? "created" : result.mode === "failed" ? "failed" : "dry_summary",
