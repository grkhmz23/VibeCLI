...
export async function buildHandoffSummary(cwd, runId) {
...
export async function writeHandoffManifest(cwd, runId) {
    const handoffPath = join(new RunStore(cwd).runPath(runId), "handoff");
    const files = [];
    for (const entry of await readdir(handoffPath).catch(() => [])) {
        if (entry === "HANDOFF_MANIFEST.json")
            continue;
        if (!entry.endsWith(".md") && !entry.endsWith(".json") && !entry.endsWith(".tar.gz"))
            continue;
    ...
    await writeFile(join(handoffPath, "HANDOFF_MANIFEST.json"), `${JSON.stringify(manifest, null, 2)}\n`, "utf8");
    return manifest;
}
export async function createHandoffBundle(cwd, runId) {
    const store = new RunStore(cwd);
...
`;
    ...
    await store.writeTextArtifact(runId, "handoff/POLICY_EXCEPTIONS.md", redactHandoffText(renderPolicyExceptions(exceptions)));
...
export async function verifyHandoffBundle(cwd, runId) {
    const handoffPath = join(new RunStore(cwd).runPath(runId), "handoff");
    const manifest = await readJson(join(handoffPath, "HANDOFF_MANIFEST.json"));
    const files = await Promise.all(manifest.files.map(async (file) => {
        const path = join(new RunStore(cwd).runPath(runId), file.path);
        if (!pathExists(path))
            return { path: file.path, ok: false, reason: "missing" };
        const hash = await sha256File(path);
        return {
            path: file.path,
            ok: hash.sha256 === file.sha256 && hash.sizeBytes === file.sizeBytes,
            reason: hash.sha256 === file.sha256 && hash.sizeBytes === file.sizeBytes ? "ok" : "hash mismatch"
        };
    }));
    const ledger = await verifyLedger(cwd, runId).catch(() => undefined);
    return {
...
export async function archiveHandoffBundle(cwd, runId) {
    const store = new RunStore(cwd);
    const handoffPath = join(store.runPath(runId), "handoff");
    const archive = {};
