...
async function validateManifest(args) {
    ...
    for (const patch of args.manifest.patches) {
        try {
            ...
            if (failed.length > 0)
                throw new Error(failed.flatMap((entry) => entry.errors).join("; "));
        }
        catch (error) {
            blocked.push({
                path: patch.path,
...
async function applyPatchFile(args) {
    ...
    if (selected.length === 0)
        throw new Error(`Patch artifact does not contain ${args.path}`);
...
async function verifyApply(args) {
    ...
    checks.push({
        ...
        ok: !args.filesChanged.some((file) => file === ".vibecli/config.yaml" || file.startsWith(".vibecli/policies/")),
...
export async function applyRun(cwd, runId, options) {
    ...
    if (blocked.length > 0) {
        ...
        await store.writeArtifact(runId, options.dryRun ? "apply-dry-run-report.json" : "apply-result.json", result);
        return result;
    }
    if (options.dryRun) {
        const result = {
    ...
    try {
        ...
        await writeFile(join(store.runPath(runId), "rollback", "post-apply.patch"), await gitDiff(cwd), "utf8");
        ...
        await store.writeTextArtifact(runId, "APPLIED_REPORT.md", createAppliedReport(result, scannerResults));
        await store.writeTextArtifact(runId, "final-report.md", createPhaseThreeReport(result, scannerResults));
        await store.writeState(state);
        return result;
    }
    catch (error) {
...
function createPhaseThreeReport(result, scannerResults) {
    const sourceModified = result.status === "applied"
    ...
    const nextAction = result.status === "applied"
...
function createAppliedReport(result, scannerResults) {
