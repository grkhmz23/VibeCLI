...
export async function getApprovalMatrix(cwd, runId, options = { write: true }) {
...
export async function addOrgApproval(cwd, runId, options) {
    ...
    if (!options.note.trim())
        throw new Error("Org approval note must be non-empty.");
    const config = await loadConfig(cwd);
...
export function computeQuorum(matrix) {
    ...
    if (rejected.length)
        blockingReasons.push("Rejected organization approval exists.");
    if (needsChanges.length)
        blockingReasons.push("needs_changes organization approval exists.");
    ...
    if (approved.length < matrix.minApprovals)
        blockingReasons.push("Minimum approvals are not met.");
    if (missingRoles.length)
        blockingReasons.push("Required reviewer roles are missing.");
...
export async function verifyApprovalMatrix(cwd, runId) {
...
async function writeMatrix(cwd, runId, matrix) {
    const store = new RunStore(cwd);
...
async function hashApprovalArtifacts(cwd, runId) {
    const store = new RunStore(cwd);
    const result = [];
...
export async function readApprovalMatrixNoteHashes(cwd, runId) {
...
//# sourceMappingURL=approval-matrix.js.map