...
export async function createEvidenceExport(cwd, runId, options = {}) {
    ...
    await store.writeTextArtifact(runId, "org/exports/EVIDENCE_EXPORT.md", renderEvidenceExport(policy, entries));
...
export async function verifyEvidenceExport(cwd, runId) {
    const store = new RunStore(cwd);
    const manifest = await readJson(join(store.runPath(runId), "org", "exports", "EVIDENCE_EXPORT_MANIFEST.json"));
    const checks = [];
...
function includedClasses(mode) {
    if (mode === "minimal")
        return ["summaries", "provenance", "ledger"];
    if (mode === "audit")
        return ["release", "handoff", "approvals", "retention", "ledger"];
    return ["release", "handoff", "provenance", "remote-attestation", "approvals", "ledger"];
}
function renderEvidenceExport(policy, files) {
...
//# sourceMappingURL=evidence-export-policy.js.map