...
export async function orgKeyStatus(cwd) {
...
export async function initOrgKey(cwd, options) {
    const expected = options.rotate ? "ROTATE ORG KEY" : "CREATE ORG KEY";
    if (options.confirm !== expected) {
        throw new Error(`Org key ${options.rotate ? "rotation" : "creation"} requires exact confirmation: ${expected}`);
    }
    const paths = await orgPaths(cwd);
    if (!options.rotate && (pathExists(paths.privateKeyPath) || pathExists(paths.publicKeyPath))) {
        throw new Error("Org signing key already exists; use --rotate with exact confirmation to rotate.");
    }
...
export async function loadOrgPrivateKey(cwd) {
    const paths = await orgPaths(cwd);
...
export async function loadOrgPublicKey(cwd) {
    const paths = await orgPaths(cwd);
...
export async function exportOrgPublicKey(cwd) {
