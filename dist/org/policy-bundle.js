...
export async function createOrgPolicyBundle(cwd, options = {}) {
    ...
    const bundle = {
        ...
        policyProfiles,
        approvalPolicies: Object.entries(config.organization.approval_policies).map(([name, policy]) => ({
            name,
        ...
        retentionPolicies: Object.entries(config.organization.retention.policies).map(([name, policy]) => ({
            name,
    ...
    await writeJson(join(paths.latestPolicyBundleDir, "ORG_POLICY_MANIFEST.json"), await createPolicyManifest(cwd));
    ...
    if (options.sign)
        await signOrgPolicyBundle(cwd);
    return bundle;
}
export async function signOrgPolicyBundle(cwd) {
    const paths = await orgPaths(cwd);
    const bundlePath = join(paths.latestPolicyBundleDir, "ORG_POLICY_BUNDLE.json");
    if (!pathExists(bundlePath))
        await createOrgPolicyBundle(cwd);
...
export async function verifyOrgPolicyBundle(cwd) {
    const paths = await orgPaths(cwd);
    const checks = [];
    const bundlePath = join(paths.latestPolicyBundleDir, "ORG_POLICY_BUNDLE.json");
    const envelopePath = join(paths.latestPolicyBundleDir, "ORG_POLICY_ENVELOPE.json");
    if (!pathExists(bundlePath))
        checks.push({ name: "bundle", ok: false, message: "missing" });
    if (!pathExists(envelopePath))
        checks.push({ name: "envelope", ok: false, message: "missing" });
    ...
    if (pathExists(bundlePath) && pathExists(envelopePath)) {
        ...
        const signatureOk = verifyCanonicalJson(bundle, envelope.signature.signatureBase64, createPublicKey(envelope.publicKey.pem));
        ...
        checks.push({
            ...
            message: envelope.signature.publicKeyFingerprint === envelope.publicKey.fingerprint
                ? "ok"
                : "fingerprint mismatch"
        });
    }
    if (pathExists(join(paths.latestPolicyBundleDir, "ORG_POLICY_MANIFEST.json"))) {
        const manifest = await readJson(join(paths.latestPolicyBundleDir, "ORG_POLICY_MANIFEST.json"));
...
export async function showOrgPolicyBundle(cwd) {
    const paths = await orgPaths(cwd);
    return readJson(join(paths.latestPolicyBundleDir, "ORG_POLICY_BUNDLE.json"));
}
async function listPolicyProfileHashes(cwd) {
...
async function createPolicyManifest(cwd) {
    const paths = await orgPaths(cwd);
    const entries = [];
    const files = await readdir(paths.latestPolicyBundleDir).catch(() => []);
    for (const file of files.filter((name) => name.startsWith("ORG_POLICY_") && name !== "ORG_POLICY_MANIFEST.json")) {
...
async function writePolicyMarkdown(cwd, bundle) {
    const paths = await orgPaths(cwd);
    await ensureDir(paths.latestPolicyBundleDir);
    await writeFile(join(paths.latestPolicyBundleDir, "ORG_POLICY_BUNDLE.md"), `# Organization Policy Bundle
...
async function writePolicySignatureMarkdown(cwd, envelope) {
    const paths = await orgPaths(cwd);
    await ensureDir(paths.latestPolicyBundleDir);
    await writeFile(join(paths.latestPolicyBundleDir, "ORG_POLICY_SIGNATURE.md"), `# Organization Policy Signature
