...
export function isSafeOrgToken(value) {
...
export function redactOrgText(value) {
...
export function validateOrganizationConfig(config) {
    const errors = [];
    if (!isSafeOrgToken(config.org_id))
        errors.push("organization.org_id must be a safe token.");
    ...
    if (looksSecret(config.org_name))
        errors.push("organization.org_name looks like a secret.");
    ...
    for (const reviewer of config.reviewers) {
        if (!isSafeOrgToken(reviewer.id))
            errors.push(`Reviewer id ${reviewer.id} is unsafe.`);
        ...
        for (const role of reviewer.roles) {
            if (!isSafeOrgToken(role))
                errors.push(`Reviewer role ${role} is unsafe.`);
        }
    }
    for (const [name, policy] of Object.entries(config.approval_policies)) {
        if (!isSafeOrgToken(name))
            errors.push(`Approval policy ${name} is unsafe.`);
        ...
        for (const role of policy.required_roles) {
            if (!isSafeOrgToken(role))
                errors.push(`Approval policy role ${role} is unsafe.`);
        }
    ...
    for (const [name, policy] of Object.entries(config.retention.policies)) {
        if (!isSafeOrgToken(name))
            errors.push(`Retention policy ${name} is unsafe.`);
    ...
    if (containsSecret(config))
        errors.push("organization config contains a secret-looking value.");
    return errors;
}
export function assertValidOrganizationConfig(config) {
