...
function operation(oldPath, newPath) {
    if (!oldPath && newPath)
        return "create";
    if (oldPath && !newPath)
        return "delete";
...
export function parseUnifiedDiff(diff) {
    ...
    for (let index = 0; index < lines.length; index += 1) {
        ...
        if (line.startsWith("--- ")) {
            ...
            if (!next.startsWith("+++ "))
                throw new Error("Malformed unified diff: missing +++ path");
            const oldPath = cleanPath(line.slice(4));
            const newPath = cleanPath(next.slice(4));
            const path = newPath ?? oldPath;
            if (!path)
                throw new Error("Malformed unified diff: missing file path");
            current = {
                oldPath,
                newPath,
                path,
        ...
        if (line.startsWith("@@ ")) {
            if (!current)
                throw new Error("Malformed unified diff: hunk before file header");
    ...
    if (files.length === 0 && diff.trim())
        throw new Error("No unified diff file headers found");
    return files;
}
