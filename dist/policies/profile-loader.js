...
import { defaultPolicyProfiles, policyProfileSchema } from "./profile.js";
export const policyProfilesDir = join(".vibecli", "policies", "profiles");
export async function ensureDefaultPolicyProfiles(cwd, force = false) {
    ...
    for (const [name, policy] of Object.entries(defaultPolicyProfiles)) {
        ...
        const existing = YAML.parse(await readFile(path, "utf8"));
        if (!existing)
            continue;
        const upgraded = {
            ...existing,
...
export async function loadPolicyProfile(cwd, name) {
...
export async function listPolicyProfileNames(cwd) {
    const dir = join(cwd, policyProfilesDir);
    if (!pathExists(dir))
        return [];
