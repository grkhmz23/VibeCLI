...
export async function validatePolicyProfile(cwd, name) {
    const errors = [];
    try {
        ...
        if (name === "strict-enterprise") {
            ...
            if (!profile.audit.require_signed_audit_export_for_production) {
                errors.push("strict-enterprise cannot disable signed audit export for production audit handoff");
            }
            if (!profile.evidence_lifecycle.require_ledger_pass_for_production_archive) {
                errors.push("strict-enterprise cannot disable ledger pass requirement for production archives");
            }
...
export async function validatePolicyProfiles(cwd, name) {
    ...
    const results = await Promise.all(names.map((profileName) => validatePolicyProfile(cwd, profileName)));
    return [
        ...results,
