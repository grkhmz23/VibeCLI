...
export async function signProvenance(cwd, runId, options) {
    ...
    const statementJson = await readFile(join(store.runPath(runId), "provenance", "provenance-statement.json"), "utf8");
    const envelope = {
        version: 1,
    ...
    await store.writeTextArtifact(runId, "provenance/PROVENANCE_SIGNATURE.md", renderSignature(envelope));
...
function renderSignature(envelope) {
