...
export async function initProvenanceKey(cwd, options) {
    const expected = options.rotate ? "ROTATE PROVENANCE KEY" : "CREATE PROVENANCE KEY";
    if (options.confirm !== expected) {
        throw new Error(`Provenance key ${options.rotate ? "rotation" : "creation"} requires exact confirmation: ${expected}`);
    }
    const paths = await provenanceKeyPaths(cwd);
    if (!options.rotate && (pathExists(paths.privateKeyPath) || pathExists(paths.publicKeyPath))) {
        throw new Error("Provenance key already exists; use --rotate with exact confirmation to rotate.");
    }
    const previous = pathExists(paths.metadataPath)
        ? await readJson(paths.metadataPath).catch(() => undefined)
        : undefined;
...
export async function loadPrivateSigningKey(cwd) {
    const paths = await provenanceKeyPaths(cwd);
