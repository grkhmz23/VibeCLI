...
export async function generateProvenanceStatement(cwd, runId) {
    ...
    const statement = {
        ...
        metadata: {
            ...
            plannedVersion: collected.releaseSummary?.version.planned ?? state.release?.version.plannedVersion ?? null,
            ...
            deploymentReadiness: collected.deployment?.verdict ?? state.release?.deploymentReadiness.verdict ?? null,
            releaseReadiness: collected.releaseReadiness?.verdict ?? state.release?.releaseReadiness.verdict ?? null
        },
        attestations: {
            ...
            deploymentReady: collected.deployment?.verdict === "ready_to_deploy" ||
...
export function renderProvenance(statement) {
