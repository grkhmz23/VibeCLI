...
export async function verifyProvenance(cwd, runId) {
    ...
    checks.push({
        ...
        message: statementHash === envelope.statementSha256
    ...
    checks.push({
        ...
        ok: fingerprint === envelope.publicKey.fingerprint &&
            fingerprint === envelope.signature.publicKeyFingerprint,
        message: "public key fingerprint check"
    });
    const signatureOk = verifyCanonicalJson(statement, envelope.signature.signatureBase64, createPublicKey(envelope.publicKey.pem));
    checks.push({
        name: "signature",
        ok: signatureOk,
    ...
    if (!ledger)
        warnings.push("Ledger is missing.");
    checks.push({
        name: "ledger",
        ok: ledger?.ok ?? false,
