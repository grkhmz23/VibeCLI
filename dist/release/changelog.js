...
export async function generateChangelogEntry(cwd, runId) {
    const store = new RunStore(cwd);
    const state = await store.readState(runId);
    const version = await readJson(join(store.runPath(runId), "release", "version-plan.json"))
    ...
    if (/security|auth|token|secret/.test(lower))
        entry.sections.security.push(summary);
    else if (/fix|bug/.test(lower))
        entry.sections.fixed.push(summary);
    else if (/add|feature|new/.test(lower))
        entry.sections.added.push(summary);
    else
        entry.sections.changed.push(summary);
    if (appliedFiles.length)
        entry.sections.changed.push(`Updated ${appliedFiles.join(", ")}.`);
...
export async function writeChangelog(cwd, runId, options) {
    ...
    const store = new RunStore(cwd);
    const entry = await readJson(join(store.runPath(runId), "release", "changelog-entry.json")).catch(() => generateChangelogEntry(cwd, runId));
    ...
    if (pathExists(changelogPath))
        await copyFile(changelogPath, backup);
