...
async function fileContainsEnvRefs(cwd, files) {
    for (const file of files) {
        const fullPath = join(cwd, file);
        if (!pathExists(fullPath))
            continue;
        const content = await readFile(fullPath, "utf8").catch(() => "");
        if (/process\.env\.|import\.meta\.env|\$\{[A-Z0-9_]+}/.test(content))
            return true;
    }
    return false;
}
export async function evaluateDeploymentReadiness(cwd, runId, options = {}) {
    ...
    const warnings = ["No deploy command was executed."];
    const passed = [];
    if (ledger?.ok)
    ...
    if (state.apply.status === "applied")
        passed.push("source applied");
    else
        warnings.push("Source changes have not been applied.");
    ...
    if (ci?.status === "passed")
        ...
        blockingReasons.push("Production requires passed CI.");
    else
        warnings.push("CI is not passed or was not ingested.");
    if (state.apply.status === "applied" &&
    ...
    if (await fileContainsEnvRefs(cwd, state.apply.filesChanged)) {
        if (pathExists(join(cwd, ".env.example")))
            passed.push(".env.example exists for env refs");
        else
            warnings.push("Environment references changed but .env.example is missing.");
    }
    ...
    await store.writeTextArtifact(runId, "release/DEPLOYMENT_READINESS.md", renderDeploymentReadiness(result));
...
export function renderDeploymentReadiness(value) {
