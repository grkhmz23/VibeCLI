...
async function collectReleaseFiles(root, dir = root) {
    ...
    for (const entry of entries) {
        ...
        if (entry.isDirectory())
            files.push(...(await collectReleaseFiles(root, fullPath)));
        if (entry.isFile())
            files.push(fullPath);
    }
    return files;
}
export async function writeReleaseManifest(cwd, runId) {
...
export async function verifyReleaseIntegrity(cwd, runId) {
