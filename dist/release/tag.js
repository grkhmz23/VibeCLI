...
export async function tagRun(cwd, runId, options = {}) {
    ...
    if (options.deleteLocal) {
        ...
        if (await tagExists(cwd, tag))
            await execFileAsync("git", ["tag", "-d", tag], { cwd });
        result.mode = "created";
    ...
    if ((await git(["status", "--short"], cwd)) &&
    ...
    else if (!options.allowLedgerWarning &&
        ...
        const args = config.release.tags.annotated && result.messagePath
