...
export function validateReleaseConfig(config) {
    ...
    if (config.allowed_channels.length === 0)
        errors.push("allowed_channels must be non-empty");
    if (config.versioning.strategy !== "semver")
...
export function assertSafeTagName(tag) {
    ...
    if (/\s|[~^:?*[\\]/.test(tag))
        throw new Error(`Unsafe tag name: ${tag}`);
    if (tag.length > 120)
        throw new Error("Tag name must be 120 characters or fewer");
}
