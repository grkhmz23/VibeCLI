...
function replaceVersion(content, path, current, planned) {
    if (path === "package.json") {
        const value = JSON.parse(content);
        value.version = planned;
        return `${JSON.stringify(value, null, 2)}\n`;
    }
    const next = content.replace(new RegExp(`(^\\s*version\\s*=\\s*")${current.replaceAll(".", "\\.")}(")`, "m"), `$1${planned}$2`);
    if (next === content)
        throw new Error(`Could not replace version in ${path}`);
    return next;
}
export async function applyVersionPlan(cwd, runId, options) {
    if (options.confirm !== `APPLY VERSION ${runId}`) {
        throw new Error(`Version apply requires exact confirmation: APPLY VERSION ${runId}`);
    }
    const store = new RunStore(cwd);
    const plan = await readJson(join(store.runPath(runId), "release", "version-plan.json"));
    ...
    try {
        for (const file of plan.files) {
            ...
            if (!pathExists(fullPath))
                throw new Error(`Version file is missing: ${file.path}`);
            const backup = join(store.runPath(runId), rollbackPath, file.path);
            await mkdir(dirname(backup), { recursive: true });
            await copyFile(fullPath, backup);
            await writeFile(fullPath, replaceVersion(await readFile(fullPath, "utf8"), file.path, file.currentVersion, file.plannedVersion), "utf8");
...
//# sourceMappingURL=version-apply.js.map