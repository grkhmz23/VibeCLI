...
export async function createAttestationExport(cwd, runId) {
    ...
    const deploymentReadiness = await optionalJson(join(runPath, "release", "deployment-readiness.json"));
    const ci = await optionalJson(join(runPath, "release", "ci-status.json"));
    const envelope = await optionalJson(join(runPath, "provenance", "provenance-envelope.json"));
    const evidence = await optionalJson(join(runPath, "evidence", "EVIDENCE_MANIFEST.json"));
    const checksums = await optionalJson(join(runPath, "provenance", "checksums.json"));
    const exceptions = await readPolicyExceptions(cwd, runId).catch(() => ({
        exceptions: []
    }));
    const ledger = await verifyLedger(cwd, runId).catch(() => undefined);
    const warnings = [];
    const statementHash = await optionalHash(join(runPath, "provenance", "provenance-statement.json"));
    const envelopeHash = await optionalHash(join(runPath, "provenance", "provenance-envelope.json"));
    const evidenceManifestHash = await optionalHash(join(runPath, "evidence", "EVIDENCE_MANIFEST.json"));
    const ledgerHash = await optionalHash(join(runPath, "ledger-manifest.json"));
    if (!envelopeHash)
        warnings.push("Signed provenance envelope is missing.");
    if (!evidenceManifestHash)
        warnings.push("Evidence bundle is missing.");
    if (!ledger?.ok)
        warnings.push("Ledger verification failed or ledger is missing.");
    const exportValue = redactAttestationJson({
        version: 1,
        ...
        release: {
            ...
            plannedVersion: releaseSummary?.version?.planned ?? state.release?.version.plannedVersion ?? null,
            tag: releaseSummary?.git?.tag ?? state.release?.tag.tag ?? null,
            releaseReadiness: releaseReadiness?.verdict ?? state.release?.releaseReadiness.verdict ?? null,
            deploymentReadiness: deploymentReadiness?.verdict ?? state.release?.deploymentReadiness.verdict ?? null
        },
        git: {
        ...
        provenance: {
            ...
            publicKeyFingerprint: envelope?.signature?.publicKeyFingerprint ?? envelope?.publicKey?.fingerprint ?? null
        },
        evidence: {
    ...
    const payload = redactAttestationJson({
        version: 1,
    ...
    await store.writeTextArtifact(runId, "remote-attestation/ATTESTATION_EXPORT.md", renderExport(exportValue));
...
export async function writeExportManifest(cwd, runId) {
    const store = new RunStore(cwd);
    const runPath = store.runPath(runId);
    const files = [];
    for (const file of [
        ...
        if (pathExists(fullPath))
            files.push({ path: file, ...(await sha256File(fullPath)) });
    }
    const base = { runId, createdAt: new Date().toISOString(), algorithm: "sha256", files };
