...
export async function refreshReceipt(cwd, runId, options = {}) {
    ...
    if (!receipt)
        base.errors.push("Remote submission receipt is missing.");
    if (!receipt?.remoteUrl)
        base.errors.push("No remote receipt URL or lookup template is available.");
    if (!options.verifyRemote) {
        if (options.dryRun)
            base.warnings.push("Dry run only; no remote endpoint was called.");
    ...
    if (options.confirm !== `VERIFY REMOTE RECEIPT ${runId}`) {
        throw new Error(`Remote receipt verification requires exact confirmation: VERIFY REMOTE RECEIPT ${runId}`);
    }
    if (base.errors.length)
        return writeReceiptVerification(cwd, runId, base, false);
    ...
    const result = await (options.get ?? getText)(receipt?.remoteUrl ?? "", headers, config.remote_attestation.request_timeout_ms);
    let remoteReceiptId = null;
    let remoteEntryHash = null;
    try {
...
async function writeReceiptVerification(cwd, runId, value, dryRun = false) {
    const store = new RunStore(cwd);
    await store.writeArtifact(runId, "remote-attestation/REMOTE_RECEIPT_VERIFICATION.json", value);
    await store.writeTextArtifact(runId, "remote-attestation/REMOTE_RECEIPT_VERIFICATION.md", `# Remote Receipt Verification
...
export async function readReceiptVerification(cwd, runId) {
    const path = join(new RunStore(cwd).runPath(runId), "remote-attestation", "REMOTE_RECEIPT_VERIFICATION.json");
