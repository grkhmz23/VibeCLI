...
export async function generateRegistryMetadata(cwd, runId, options = {}) {
    if (options.image && !imagePattern.test(options.image))
        throw new Error("Unsafe image name.");
    if (options.tag && !tagPattern.test(options.tag))
        throw new Error("Unsafe image tag.");
    const store = new RunStore(cwd);
    const state = await store.readState(runId);
    const runPath = store.runPath(runId);
    const releaseSummary = pathExists(join(runPath, "release", "RELEASE_SUMMARY.json"))
        ? await readJson(join(runPath, "release", "RELEASE_SUMMARY.json")).catch(() => undefined)
        : undefined;
    ...
    const metadata = redactAttestationJson({
        runId,
        createdAt: created,
        format: "oci-inspired",
        image: {
        ...
        annotations: {
            ...
            "org.opencontainers.image.description": "VibeCLI release metadata generated for manual operator use.",
    ...
    await store.writeTextArtifact(runId, "remote-attestation/OCI_ANNOTATIONS.env", renderEnv(metadata));
    await store.writeTextArtifact(runId, "remote-attestation/REGISTRY_METADATA.md", renderMetadata(metadata));
