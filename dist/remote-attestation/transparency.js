...
export async function generateTransparencyEntry(cwd, runId) {
    ...
    const attestation = await optionalJson(join(runPath, "remote-attestation", "ATTESTATION_EXPORT.json"));
    const provenanceStatement = await optionalJson(join(runPath, "provenance", "provenance-statement.json"));
    ...
    const ledgerHash = pathExists(join(runPath, "ledger-manifest.json"))
        ...
        : null;
    const warnings = [];
    if (!attestation?.provenance?.signed)
        warnings.push("Signed provenance is missing.");
    if (!attestation?.evidence?.manifestSha256)
        warnings.push("Evidence manifest is missing.");
    if (!ledger?.ok)
    ...
    const base = redactAttestationJson({
        ...
        subjects: provenanceStatement?.subject?.map((subject) => ({
            name: subject.name,
        ...
        attestations: {
            ...
            scannerHighCriticalClear: !state.scanners || state.scanners.criticalFindings + state.scanners.highFindings === 0,
            ciPassed: state.release?.ci.status === "passed",
            deploymentReady: state.release?.deploymentReadiness.verdict === "ready_to_deploy" ||
    ...
    await store.writeTextArtifact(runId, "remote-attestation/TRANSPARENCY_ENTRY.md", renderEntry(entry));
...
export async function appendTransparencyEntry(cwd, runId, confirm) {
    if (confirm !== `APPEND TRANSPARENCY ${runId}`) {
        throw new Error(`Transparency append requires exact confirmation: APPEND TRANSPARENCY ${runId}`);
    }
    const store = new RunStore(cwd);
    const entry = pathExists(join(store.runPath(runId), "remote-attestation", "TRANSPARENCY_ENTRY.json"))
        ? await readJson(join(store.runPath(runId), "remote-attestation", "TRANSPARENCY_ENTRY.json"))
    ...
    const chain = pathExists(chainPath) ? await readJson(chainPath) : [];
...
export async function verifyTransparencyLog(cwd, runId) {
    const chainPath = join(cwd, ".vibecli", "transparency-log", "chain.json");
    if (!pathExists(chainPath))
        return { ok: false, checks: [], foundRun: false };
    const chain = await readJson(chainPath).catch(() => undefined);
    if (!chain)
        return { ok: false, checks: [], foundRun: false };
