...
function providerAvailable(config, providerName) {
    const provider = config.providers[providerName];
    if (!provider)
        return { available: false, reason: `Provider ${providerName} is not configured` };
...
export function routeAgent(config, profile, agent) {
    const agentConfig = config.profiles[profile]?.agents[agent];
    if (!agentConfig)
        throw new Error(`Agent ${agent} is not configured in profile ${profile}`);
    const strategy = routingStrategyForProfile(config, profile);
    const primary = resolveAgentModelRef(config, agentConfig.provider, agentConfig.model, agentConfig.model_alias);
    ...
    const selected = primaryAvailability.available || !fallbackAllowed
        ? {
