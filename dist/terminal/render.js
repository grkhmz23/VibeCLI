...
export function renderSafetyBanner(theme, columns) {
    return [
        line(columns),
...
export function renderHeader(info, theme, columns) {
    const providers = info.providers
        .map((provider) => `${provider.name}:${provider.status === "configured" ? theme.green("ok") : theme.yellow("warn")}`)
        .join(" ");
    return [
        renderLogo(theme),
        "",
...
export function renderInputFrame(theme, columns) {
    return [
        line(columns),
...
export function renderAgentStatusBoard(state, theme) {
...
export function renderRunSummary(state, theme) {
    return [
...
export function renderReviewSummary(value) {
    return value;
}
export function renderCommandResult(value, theme) {
