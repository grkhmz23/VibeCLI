...
export function redactSecrets(output) {
    return output
        .replace(/(OPENAI_API_KEY|ANTHROPIC_API_KEY|OPENROUTER_API_KEY|API_KEY|SECRET|TOKEN|PASSWORD)=\S+/g, "$1=[REDACTED]")
        .replace(/\b[A-Za-z0-9+/=_-]{48,}\b/g, "[REDACTED]");
}
function execAllowed(binary, args, cwd, timeoutMs) {
...
export async function executeAllowedCommand(command, cwd) {
...
export async function executeAllowedCommandDetailed(command, cwd, timeoutMs = 60_000) {
    const classification = classifyCommand(command);
    if (classification.classification !== "allowed") {
        return {
            command,
