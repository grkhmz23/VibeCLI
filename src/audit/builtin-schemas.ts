import type { AuditControl, AuditControlCategory, AuditSchema } from "./types.js";
...
const categoryDetails: Array<{
  ...
  sources: AuditControl["evidenceSources"];
}> = [
  {
    category: "security",
    id: "SEC-BASELINE-001",
...
export function builtinAuditSchemas(): AuditSchema[] {
  return [
    makeSchema(
      "internal-secure-release",
      "Internal Secure Release Evidence Mapping",
      "Read-only evidence mapping for release governance."
    ),
    makeSchema(
      "startup-production-readiness",
      "Startup Production Readiness Mapping",
      "Read-only release evidence mapping for small teams preparing production releases."
    ),
    makeSchema(
      "soc2-security-mapping",
      "SOC 2 Security Control Mapping Support",
      "Read-only evidence mapping that can support security control discussions."
    ),
    makeSchema(
      "iso27001-secure-sdlc-mapping",
      "ISO 27001 Secure SDLC Mapping Support",
      "Read-only evidence mapping that can support secure SDLC control discussions."
    ),
    makeSchema(
      "ai-appsec-readiness",
      "AI Application Security Readiness Mapping",
      "Read-only evidence mapping focused on AI provider safety and application security."
    ),
    makeSchema(
      "crypto-fintech-release-readiness",
      "Crypto Fintech Release Readiness Mapping",
      "Read-only evidence mapping for sensitive financial application release workflows."
    )
  ];
}

export function builtinAuditSchema(name: string): AuditSchema | undefined {
  return builtinAuditSchemas().find((schema) => schema.name === name);
}

function makeSchema(name: string, title: string, description: string): AuditSchema {
  return {
    version: 1,
    name,
    title,
      ...
      checks: [
        {
          ...
          description:
            "Required evidence is present, redacted, and tied to local VibeCLI artifacts.",
