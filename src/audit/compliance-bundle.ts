...
export async function createComplianceBundle(
  cwd: string,
  runId: string,
  options: { schema?: string; minimal?: boolean; sign?: boolean; confirm?: string } = {}
): Promise<ComplianceCheckBundle> {
  if (options.sign && options.confirm !== `SIGN COMPLIANCE BUNDLE ${runId}`) {
    throw new Error(
      `Compliance bundle signing requires exact confirmation: SIGN COMPLIANCE BUNDLE ${runId}`
    );
  }
  ...
  const bundle: ComplianceCheckBundle = {
    version: 1,
  ...
  await store.writeTextArtifact(
    runId,
    "audit/compliance/COMPLIANCE_CHECK_BUNDLE.md",
    renderBundle(bundle)
  );
  const archivePath = join(
    ...
    "audit",
    "compliance",
    `vibecli-compliance-${runId}-${coverage.schema}.tar.gz`
  );
  ...
  if (options.sign) {
    ...
    const envelope = makeSignedAuditEnvelope(
      runId,
      "vibecli.compliance.bundle-envelope",
    ...
    await store.writeTextArtifact(
      runId,
      "audit/compliance/COMPLIANCE_BUNDLE_SIGNATURE.md",
      `# Compliance Bundle Signature\n\nPublic key fingerprint: ${envelope.publicKey.fingerprint}\n`
    );
...
export async function verifyComplianceBundle(cwd: string, runId: string) {
  const store = new RunStore(cwd);
  const root = join(store.runPath(runId), "audit", "compliance");
  const result = await verifyAuditManifest(
    root,
    join(root, "COMPLIANCE_MANIFEST.json"),
    join(root, "COMPLIANCE_BUNDLE_ENVELOPE.json"),
    join(root, "COMPLIANCE_CHECK_BUNDLE.json")
  );
  await updateAuditState(store, runId, (audit) => {
...
async function writeComplianceManifest(
  cwd: string,
  runId: string,
  schema: string,
...
function renderBundle(bundle: ComplianceCheckBundle): string {
