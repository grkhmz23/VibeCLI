...
export function evaluateControl(
  ...
  evidenceBySource: Map<string, AuditEvidenceRef[]>
): AuditMappedControl {
  const evidence = control.evidenceSources.flatMap((source) => evidenceBySource.get(source) ?? []);
  const missingSources = control.evidenceSources.filter(
    (source) => (evidenceBySource.get(source) ?? []).length === 0
  );
  const status =
    evidence.length === 0
      ? control.required
...
function recommendationFor(source: string): string {
