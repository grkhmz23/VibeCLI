...
const sourcePaths: Record<AuditEvidenceSourceName, string[]> = {
...
export async function collectAuditEvidence(
  cwd: string,
  runId: string,
  source: AuditEvidenceSourceName
): Promise<AuditEvidenceRef[]> {
...
function summarizeEvidence(path: string, content: string): string {
