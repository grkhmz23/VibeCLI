...
import type { AuditEvidenceMap, AuditGapPriority, AuditGapsReport } from "./types.js";

export async function generateAuditGaps(
  cwd: string,
  runId: string,
...
function priorityFor(severity: string, required: boolean, status: string): AuditGapPriority {
  if (required && severity === "critical" && status === "missing") return "p0";
  if (required && severity === "high" && status === "missing") return "p1";
  if (required && (severity === "medium" || status === "partial")) return "p2";
  return "p3";
}

function safeCommandsFor(category: string): string[] {
...
function renderGaps(report: AuditGapsReport): string {
  return `# Audit Gaps
...
${report.gaps
  .map(
    (gap) =>
      `- ${gap.priority.toUpperCase()} ${gap.controlId}: ${gap.title}\n  Commands: ${gap.recommendedVibeCommands.join("; ")}`
  )
  .join("\n")}
`;
}
