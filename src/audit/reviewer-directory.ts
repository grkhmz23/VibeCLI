...
export async function readReviewerDirectoryFile(
  cwd: string,
  file: string,
  options: { allowRawEmail?: boolean; confirm?: string } = {}
): Promise<{ reviewers: ReviewerDirectoryEntry[]; errors: string[]; rawEmailsHashed: boolean }> {
  ...
  const reviewers: ReviewerDirectoryEntry[] = [];
  const errors: string[] = [];
  let rawEmailsHashed = false;
  for (const [index, raw] of parsed.entries()) {
    const entry = raw as Record<string, unknown>;
    const id = stringValue(entry.id);
    const displayName = redactReleaseText(
      stringValue(entry.displayName ?? entry.display_name),
      120
    );
    ...
    if (!safeToken.test(id)) errors.push(`Reviewer ${index} has unsafe id.`);
    if (!displayName || displayName.length > 120)
      errors.push(`Reviewer ${id || index} has invalid display name.`);
    for (const role of roles) {
      if (!safeToken.test(role)) errors.push(`Reviewer ${id || index} has unsafe role ${role}.`);
    }
    ...
    if (typeof entry.email === "string" && entry.email) {
      if (options.allowRawEmail && options.confirm !== "IMPORT RAW REVIEWER EMAILS") {
        throw new Error(
          "Raw reviewer email import requires exact confirmation: IMPORT RAW REVIEWER EMAILS"
        );
      }
