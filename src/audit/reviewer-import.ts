...
export async function previewReviewerImport(
  cwd: string,
  file: string,
  ...
  for (const reviewer of parsed.reviewers) {
    if (seen.has(reviewer.id)) duplicates.push(reviewer.id);
    seen.add(reviewer.id);
  }
  const preview: ReviewerImportPreview = {
  ...
  await writeFile(
    join(cwd, ".vibecli/audit/reviewer-directory/import-preview.json"),
...
export async function applyReviewerImport(
  cwd: string,
  file: string,
  options: { confirm?: string; allowRawEmail?: boolean; rawEmailConfirm?: string } = {}
): Promise<ReviewerImportPreview> {
  ...
  const raw = parse(configText) as Record<string, unknown>;
