...
const certificationClaim =
  /\b(certified compliant|certifies compliance|is compliant with|guarantees compliance|attests compliance)\b/i;

export const auditCategories: AuditControlCategory[] = [
  "security",
  "authentication",
  "authorization",
  "secrets",
...
export const auditSeverities: AuditControlSeverity[] = ["low", "medium", "high", "critical"];

export function validateAuditConfig(config: AuditConfig): string[] {
  const errors: string[] = [];
  ...
  if (!config.compliance_language.avoid_certification_claims) {
    errors.push(
      "audit.compliance_language.avoid_certification_claims must remain true in Phase 13"
    );
  }
  ...
  for (const [key, value] of Object.entries(flatten(config))) {
    if (secretLike.test(String(value))) errors.push(`audit.${key} looks like a raw secret`);
  }
  return errors;
}

export function validateAuditSchema(schema: AuditSchema): string[] {
  const errors: string[] = [];
  const text = JSON.stringify(schema);
  if (secretLike.test(text)) errors.push("audit schema contains a secret-looking value");
  if (certificationClaim.test(text))
    errors.push("audit schema must not claim certification or compliance");
  if (schema.version !== 1) errors.push("audit schema version must be 1");
  if (!safeToken.test(schema.name)) errors.push("audit schema name must be a safe lowercase token");
  ...
  for (const control of schema.controls ?? []) {
    if (!safeControlId.test(control.id)) errors.push(`audit control id is unsafe: ${control.id}`);
    if (ids.has(control.id)) errors.push(`duplicate audit control id: ${control.id}`);
    ids.add(control.id);
    ...
    if (!control.checks?.length) errors.push(`audit control ${control.id} must include checks`);
  }
  return errors;
}

function pathUnderAudit(path: string): boolean {
