...
import {
  applyAcceptances,
  readWarningAcceptances,
  writeWarningAcceptances
} from "./warning-store.js";
import type { BetaWarningRecord } from "./types.js";

export type BetaWarningsReport = {
  createdAt: string;
  warnings: BetaWarningRecord[];
  summary: { open: number; accepted: number; resolved: number; blockingOpen: number };
};

export async function collectBetaWarnings(cwd: string): Promise<BetaWarningsReport> {
  const reportsDir = join(cwd, ".vibecli", "dogfood", "reports");
  const warnings: BetaWarningRecord[] = [];
  ...
  await writeFile(
    join(cwd, ".vibecli", "beta", "reports", "BETA_WARNINGS.md"),
...
export async function acceptBetaWarning(
  cwd: string,
  id: string,
  options: { by?: string; reason?: string; confirm?: string; strict?: boolean; resolve?: boolean }
  ...
  if (!options.by?.trim()) throw new Error("Beta warning update requires --by");
  if (!options.reason?.trim()) throw new Error("Beta warning update requires --reason");
  const current = await collectBetaWarnings(cwd);
  const warning = current.warnings.find((item) => item.id === id);
  if (!warning) throw new Error(`Beta warning ${id} was not found`);
...
function summarize(warnings: BetaWarningRecord[]): BetaWarningsReport {
  return {
...
function add(
  warnings: BetaWarningRecord[],
  source: string,
  category: BetaWarningRecord["category"],
  message: string,
  options: { severity?: BetaWarningRecord["severity"]; blocking?: boolean } = {}
): void {
  warnings.push({
...
function dedupe(warnings: BetaWarningRecord[]): BetaWarningRecord[] {
...
async function collectDocsWarnings(warnings: BetaWarningRecord[], path: string): Promise<void> {
  if (!pathExists(path))
    return add(warnings, "docs-check", "docs", "Docs check has not been run.", {
      severity: "medium",
      blocking: true
    });
  const report = await readJson<{ warnings?: string[]; blockers?: string[] }>(path);
...
async function collectScannerWarnings(warnings: BetaWarningRecord[], path: string): Promise<void> {
  if (!pathExists(path))
    return add(warnings, "scanner-check", "scanner", "Scanner readiness has not been run.", {
      severity: "low"
    });
  const report = await readJson<{ summary?: { missing: number; failed: number } }>(path);
  if ((report.summary?.missing ?? 0) > 0)
    add(
      warnings,
      "scanner-check",
      "scanner",
      `${report.summary?.missing ?? 0} optional scanners missing.`,
      { severity: "low" }
    );
  if ((report.summary?.failed ?? 0) > 0)
    add(
      ...
      "scanner-check",
      "scanner",
      `${report.summary?.failed ?? 0} scanner checks failed.`,
      { severity: "medium" }
    );
}

async function collectPackageWarnings(warnings: BetaWarningRecord[], path: string): Promise<void> {
  if (!pathExists(path))
...
async function collectBetaWarningsFromReadiness(
  warnings: BetaWarningRecord[],
  path: string
): Promise<void> {
...
async function collectBacklogWarnings(warnings: BetaWarningRecord[], path: string): Promise<void> {
  if (!pathExists(path)) return;
  const report = await readJson<{
    items?: Array<{ title: string; priority: string; blockingBeta: boolean }>;
  }>(path);
...
function collectDogfoodWarnings(warnings: BetaWarningRecord[], reportsDir: string): void {
