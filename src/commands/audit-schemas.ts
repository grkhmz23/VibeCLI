...
export function registerAuditSchemasCommand(program: Command): void {
  ...
  command
    ...
    .action(async (name: string | undefined, options: { json?: boolean }) => {
      ...
      if (result.some((entry) => !entry.ok)) process.exitCode = 1;
    });
  command
    ...
    .action(async (options: { force?: boolean; confirm?: string }) => {
      console.log(
        JSON.stringify(await installDefaultAuditSchemas(process.cwd(), options), null, 2)
      );
    });
}
