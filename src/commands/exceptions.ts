...
import {
  ...
  writePolicyExceptions,
  type PolicyException
} from "../handoff/policy-exceptions.js";

export function registerExceptionsCommand(program: Command): void {
  program
    .command("exceptions")
    ...
    .action(
      async (
        ...
        options: {
          request?: string;
          policy?: string;
          severity?: PolicyException["severity"];
          mitigation?: string;
          approve?: string;
          reject?: string;
          by?: string;
        ...
        if (options.request !== undefined) {
          ...
          if (
            ...
            throw new Error(
              `Critical exception requires exact confirmation: REQUEST CRITICAL EXCEPTION ${runId}`
            );
          }
