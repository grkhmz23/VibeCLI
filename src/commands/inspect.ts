...
export function registerInspectCommand(program: Command): void {
  program
    ...
    .action(async (runId: string, options: { json?: boolean }) => {
      ...
      console.log(
        `Organization: enabled=${state.organization?.enabled ?? false} approvals=${state.organization?.approvals.status ?? "not_started"} retention=${state.organization?.retention.status ?? "not_started"} audit=${state.organization?.audit.status ?? "not_started"}`
      );
      console.log("Agents:");
