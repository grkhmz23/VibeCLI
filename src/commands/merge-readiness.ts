...
export function registerMergeReadinessCommand(program: Command): void {
  const action = async (
    runId: string,
    options: { github?: boolean; pr?: string; confirm?: string; json?: boolean }
  ): Promise<void> => {
    ...
    console.log(
      options.json
        ...
        : `Merge readiness ${runId}: ${result.verdict}\nBlocked: ${
            result.blockingReasons.join("; ") || "none"
          }`
    );
  };
  program
