...
export function registerProvenanceCommand(program: Command): void {
  ...
  key
    ...
    .action(async (rawOptions: { json?: boolean }, command: Command) => {
  ...
  key
    ...
    .action(async (rawOptions: { confirm?: string; rotate?: boolean }, command: Command) => {
      const options = actionOptions(rawOptions, command);
...
function actionOptions<T extends object>(options: T, command: Command): T {
  return Object.keys(options).length ? options : command.opts<T>();
}

function readUnknownOption(args: string[], flag: string): string | undefined {
  const index = args.indexOf(flag);
  return index >= 0 ? args[index + 1] : undefined;
}

function renderKeyStatus(status: Awaited<ReturnType<typeof keyStatus>>): string {
  return [
