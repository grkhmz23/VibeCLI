...
export function registerProvidersCommand(program: Command): void {
  ...
  providers
    ...
    .action(async (options: { models?: boolean; json?: boolean }) => {
      const config = await loadConfig(process.cwd());
      ...
      for (const item of diagnostics) {
        console.log(
          `${item.provider}\t${item.type}\tenv:${item.env}\thealth:${item.health}\trun:${item.supportsRunAgent ? "yes" : "no"}\tstream:${item.supportsStreamAgent ? "yes" : "no"}${item.models === undefined ? "" : `\tmodels:${item.models ?? "error"}`} - ${item.message}`
        );
      }
    });
  ...
  add
    ...
    .action(async (options: { name: string; baseUrl: string; apiKeyEnv: string }) => {
      const config = await loadConfig(process.cwd());
