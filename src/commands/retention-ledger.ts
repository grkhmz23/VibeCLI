...
import type { RetentionLedgerEvent } from "../evidence-lifecycle/types.js";

export function registerRetentionLedgerCommand(program: Command): void {
  program
    ...
    .action(
      async (
        ...
        if (options.record) {
          if (!runId) throw new Error("Manual retention ledger record requires a run id.");
