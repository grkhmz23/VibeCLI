...
function isAgent(value: string): value is AgentRoleId {
  return (agentRoleIds as readonly string[]).includes(value);
}
...
export function registerRouteCommand(program: Command): void {
  program
    ...
    .action(
      async (options: { agent?: string; profile?: string; policy?: string; json?: boolean }) => {
        const config = await loadConfig(process.cwd());
        const profile = options.profile ?? config.default_profile;
        if (options.agent) {
          if (!isAgent(options.agent)) throw new Error(`Unknown agent ${options.agent}`);
          const route = routeAgent(config, profile, options.agent);
          if (options.json) console.log(JSON.stringify(route, null, 2));
          else {
            console.log(`${route.agent}: ${route.selected.provider}/${route.selected.model}`);
            console.log(`Reason: ${route.selected.reason}`);
            for (const fallback of route.fallbacks) {
              console.log(
                `Fallback: ${fallback.provider}/${fallback.model} ${fallback.available ? "available" : "unavailable"} - ${fallback.reason}`
              );
            }
          }
          return;
