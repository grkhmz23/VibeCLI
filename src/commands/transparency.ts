...
export function registerTransparencyCommand(program: Command): void {
  const command = program.command("transparency").description("Manage local transparency entries");
  command.argument("[run-id]", "run id").action(async (runId?: string) => {
    if (!runId) throw new Error("Usage: vibe transparency <run-id>");
  ...
  command
    ...
    .action(async (runId: string, options: { confirm?: string }) => {
      console.log(
        JSON.stringify(
          await appendTransparencyEntry(process.cwd(), runId, options.confirm),
          null,
          2
        )
      );
