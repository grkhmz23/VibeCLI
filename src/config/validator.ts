...
export async function validateTeamConfig(
  cwd: string,
  config: VibeConfig
  ...
  for (const [profileName, profile] of Object.entries(config.profiles)) {
    for (const [agent, agentConfig] of Object.entries(profile.agents)) {
      if (!config.providers[agentConfig.provider]) {
        errors.push(
          `Profile ${profileName} agent ${agent} references missing provider ${agentConfig.provider}`
        );
      }
      ...
      if (agentConfig.model_alias && !config.model_aliases[agentConfig.model_alias]) {
        errors.push(
          `Profile ${profileName} agent ${agent} references missing alias ${agentConfig.model_alias}`
        );
      }
      for (const fallback of agentConfig.fallback_models ?? []) {
        ...
        if (!provider || !config.providers[provider]) {
          errors.push(
            `Profile ${profileName} agent ${agent} fallback references missing provider ${provider ?? "unknown"}`
          );
        }
        if (fallback.model && fallback.model_alias) {
          errors.push(
            `Profile ${profileName} agent ${agent} fallback sets both model and model_alias`
          );
        }
      }
    }
  ...
  for (const [name, provider] of Object.entries(config.providers)) {
    if ("api_key_env" in provider) {
      if (secretLike.test(provider.api_key_env)) {
        errors.push(
          `Provider ${name} api_key_env looks like an actual secret; store only env var names`
        );
      }
  ...
  try {
    ...
    errors.push(
      `git_lifecycle.branch_prefix is unsafe: ${
  ...
  for (const branch of config.git_lifecycle.protected_branches) {
    try {
      ...
      errors.push(
        `git_lifecycle.protected_branches contains unsafe branch ${branch}: ${
          error instanceof Error ? error.message : String(error)
        }`
      );
    }
  ...
  try {
    ...
    errors.push(
      `release.release_branch.prefix is unsafe: ${
        error instanceof Error ? error.message : String(error)
      }`
    );
  }
  try {
    ...
    errors.push(
      `release.tags.prefix is unsafe: ${error instanceof Error ? error.message : String(error)}`
    );
  }
  ...
  if (
    config.provenance.signing.key_dir !== ".vibecli/keys" &&
  ...
  if (
    config.provenance.github_release.allow_draft_creation &&
  ...
  if (policy) {
    for (const key of Object.keys(requiredSecurityPolicy)) {
      if (!(key in policy)) errors.push(`Security policy missing required check ${key}`);
    }
  }
  for (const pattern of requiredDangerousPatterns) {
    if (!pattern) errors.push("Invalid required command denylist pattern");
  }
