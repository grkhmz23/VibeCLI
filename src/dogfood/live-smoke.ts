...
export async function previewLiveSmoke(
  cwd: string,
...
export async function runLiveSmoke(
  cwd: string,
  options: { provider?: string; model?: string; profile?: string; confirm?: string }
  ...
  if (!providerConfig) throw new Error(`Provider ${providerName} is not configured`);
...
export async function runLiveRcSmoke(
  cwd: string,
  options: { profile?: string; confirm?: string }
  ...
  const providers: LiveRcSmokeResult["providers"] = [];
  ...
  const result: LiveRcSmokeResult = {
    createdAt: new Date().toISOString(),
    status: failures.length ? "failed" : passed ? "passed" : "skipped",
    profile,
    providers,
    totalUsage: {
      totalTokens: providers.reduce<number | null>((sum, provider) => {
        if (sum === null && provider.usage.totalTokens === null) return null;
...
async function writeLiveSmoke(cwd: string, result: LiveSmokeResult): Promise<void> {
  const dir = join(cwd, ".vibecli", "dogfood", "reports", "live-smoke");
  const jsonPath = join(dir, "LIVE_SMOKE.json");
  await writeJson(jsonPath, result);
  await import("node:fs/promises").then((fs) =>
    fs.writeFile(
      join(dir, "LIVE_SMOKE.md"),
