...
function passes(
  ...
  severity: "low" | "medium" | "high" | "critical",
  fn: () => void
): SecurityRedteamReport["checks"][number] {
  try {
    fn();
...
function expectThrows(fn: () => void): void {
  ...
  if (!threw) throw new Error("Expected safety guard to reject input");
}

export async function runSecurityRedteam(cwd: string): Promise<SecurityRedteamReport> {
  const checks: SecurityRedteamReport["checks"] = [
    passes("path traversal blocked in patch paths", "critical", () =>
      expectThrows(() => validateSourcePath("../outside.ts", { repoRoot: cwd }))
    ),
    passes("absolute paths blocked", "critical", () =>
      expectThrows(() => validateSourcePath("/tmp/outside.ts", { repoRoot: cwd }))
    ),
    passes(".env patch blocked", "critical", () =>
      expectThrows(() => validateSourcePath(".env", { repoRoot: cwd }))
    ),
    passes("private key patch blocked", "critical", () =>
      expectThrows(() => validatePatchContent("-----BEGIN PRIVATE KEY-----secret"))
    ),
    passes("protected .vibecli/policies path blocked", "high", () =>
      expectThrows(() =>
        validateSourcePath(".vibecli/policies/security-policy.yaml", { repoRoot: cwd })
      )
    ),
    passes("dangerous command denied", "critical", () => {
      const result = classifyCommand("rm -rf .");
      if (result.classification !== "denied") throw new Error("dangerous command was not blocked");
    }),
    passes("env-printing command denied", "high", () => {
      const result = classifyCommand("env");
      if (result.classification !== "denied") throw new Error("env command was not blocked");
    }),
    ...
    passes(
      "remote attestation SSRF URL validation blocks local/private IP targets",
      "critical",
      () => {
        ...
        if (errors.length === 0) throw new Error("local URL was not rejected");
      }
    ),
  ...
  const report: SecurityRedteamReport = {
    ...
    blockers: checks
      .filter(
        (check) =>
          check.status === "failed" && (check.severity === "critical" || check.severity === "high")
      )
  ...
  await import("node:fs/promises").then((fs) =>
    fs.writeFile(
      join(cwd, ".vibecli", "dogfood", "reports", "SECURITY_REDTEAM.md"),
