...
export async function createDisposalAttestation(
  cwd: string,
  runId: string,
  ...
  if (options.sign && options.confirm !== `SIGN DISPOSAL ATTESTATION ${runId}`) {
    throw new Error(
      `Disposal attestation signing requires exact confirmation: SIGN DISPOSAL ATTESTATION ${runId}`
    );
  }
  ...
  const payload = {
    version: 1 as const,
    type: "vibecli.disposal.attestation" as const,
    runId,
    ...
    scope: "run-evidence-only" as const,
    preDeleteRequirementsSatisfied: plan.preDeleteRequirements.every(
      (requirement) =>
        !requirement.required ||
        requirement.satisfied ||
        requirement.name === "disposal_attestation"
    ),
    legalHold: plan.preDeleteRequirements.some(
      (requirement) => requirement.name === "no_legal_hold" && !requirement.satisfied
    ),
    archiveVerified: plan.preDeleteRequirements.some(
      (requirement) => requirement.name === "archive_verified" && requirement.satisfied
    ),
    ledgerVerified: plan.preDeleteRequirements.some(
      (requirement) => requirement.name === "ledger_verified" && requirement.satisfied
    ),
  ...
  const attestation: DisposalAttestation = {
    ...payload,
    signature: key
      ? {
          algorithm: "ed25519",
  ...
  await store.writeArtifact(
    runId,
    "evidence-lifecycle/disposal/DISPOSAL_ATTESTATION.json",
    attestation
  );
  await store.writeTextArtifact(
    runId,
    "evidence-lifecycle/disposal/DISPOSAL_ATTESTATION.md",
...
function renderAttestation(attestation: DisposalAttestation): string {
