...
import {
  classifyDisposalPath,
  conservativeCandidateReason,
  isProtectedDisposalPath
} from "./classifier.js";
import type { DisposalCandidates } from "./types.js";

export async function buildDisposalCandidates(
  cwd: string,
  runId: string
): Promise<DisposalCandidates> {
  const store = new RunStore(cwd);
  const runPath = store.runPath(runId);
  const files = await collectFiles(runPath);
  const candidates: DisposalCandidates["candidates"] = [];
  const blocked: DisposalCandidates["blocked"] = [];
  ...
  const blockedBytes = await blockedSize(
    runPath,
    blocked.map((entry) => entry.path)
  );
  const result: DisposalCandidates = {
    runId,
  ...
  await store.writeTextArtifact(
    runId,
    "evidence-lifecycle/disposal/DISPOSAL_CANDIDATES.md",
...
async function blockedSize(runPath: string, paths: string[]): Promise<number> {
  let total = 0;
...
function renderCandidates(result: DisposalCandidates): string {
