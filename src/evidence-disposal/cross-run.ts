...
import type { DisposalCandidates, DisposalEligibility, DisposalReceipt } from "./types.js";
...
export async function createDisposalReport(
  cwd: string,
  ...
  await import("node:fs/promises").then((fs) =>
    fs.writeFile(join(outputDir, "DISPOSAL_REPORT.md"), renderReport(report), "utf8")
  );
  return report;
}

async function summarizeRun(cwd: string, runId: string): Promise<DisposalReportRun> {
  ...
  const eligibility = pathExists(join(root, "DISPOSAL_ELIGIBILITY.json"))
    ? await readJson<DisposalEligibility>(join(root, "DISPOSAL_ELIGIBILITY.json")).catch(
        () => undefined
      )
    : undefined;
  const candidates = pathExists(join(root, "DISPOSAL_CANDIDATES.json"))
    ? await readJson<DisposalCandidates>(join(root, "DISPOSAL_CANDIDATES.json")).catch(
        () => undefined
      )
    : undefined;
  const receipt = pathExists(join(root, "DISPOSAL_RECEIPT.json"))
    ? await readJson<DisposalReceipt>(join(root, "DISPOSAL_RECEIPT.json")).catch(() => undefined)
    : undefined;
  const disposalStatus: DisposalReportRun["disposalStatus"] =
    receipt?.status ??
...
function renderReport(report: Awaited<ReturnType<typeof createDisposalReport>>): string {
