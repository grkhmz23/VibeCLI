...
import type { DeletedArtifactsRecord, DisposalPlan, DisposalReceipt } from "./types.js";

export async function dryRunDisposal(
  cwd: string,
  runId: string
): Promise<{ runId: string; ok: boolean; candidates: number }> {
  const precheck = await runDisposalPrecheck(cwd, runId);
  const store = new RunStore(cwd);
  const plan = await readJson<DisposalPlan>(
    join(store.runPath(runId), "evidence-lifecycle", "disposal", "DISPOSAL_PLAN.json")
  ).catch(() => undefined);
...
export async function executeDisposal(
  ...
  const precheck = await runDisposalPrecheck(cwd, runId);
  if (!precheck.ok) throw new Error("Disposal precheck failed; no evidence was deleted.");
  const store = new RunStore(cwd);
  const runPath = store.runPath(runId);
  const plan = await readJson<DisposalPlan>(
    join(runPath, "evidence-lifecycle", "disposal", "DISPOSAL_PLAN.json")
  );
  const deleted: DisposalReceipt["deleted"] = [];
  const failed: DisposalReceipt["failed"] = [];
  const skipped: DisposalReceipt["skipped"] = [];
  const archiveVerifiedBeforeDelete =
    precheck.checks.find((check) => check.name === "archive verified")?.ok ?? false;
  const legalHoldAtDeleteTime = !precheck.checks.find(
    (check) => check.name === "legal hold not enabled"
  )?.ok;
  ...
  await store.writeTextArtifact(
    runId,
    "evidence-lifecycle/disposal/RECOVERY_GUIDANCE.md",
    recoveryGuidance(runId)
  );
  ...
  await store.writeTextArtifact(
    runId,
    "evidence-lifecycle/disposal/DISPOSAL_RECEIPT.md",
    renderReceipt(receipt)
  );
  const receiptHash = (
    await sha256File(join(runPath, "evidence-lifecycle", "disposal", "DISPOSAL_RECEIPT.json"))
  ).sha256;
  const deletedArtifacts: DeletedArtifactsRecord = {
    runId,
    createdAt: new Date().toISOString(),
  ...
  await store.writeArtifact(
    runId,
    "evidence-lifecycle/disposal/DELETED_ARTIFACTS.json",
    deletedArtifacts
  );
...
function recoveryGuidance(runId: string): string {
...
function renderReceipt(receipt: DisposalReceipt): string {
