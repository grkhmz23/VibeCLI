...
import type { DisposalPlan, DisposalPrecheck } from "./types.js";

export async function runDisposalPrecheck(cwd: string, runId: string): Promise<DisposalPrecheck> {
  ...
  const checks: DisposalPrecheck["checks"] = [];
  ...
  const plan = await readJson<DisposalPlan>(planPath);
  checks.push({
  ...
  const legalHold = pathExists(legalHoldPath)
    ? await readJson<{ status: string }>(legalHoldPath).catch(() => undefined)
    : undefined;
  checks.push({
  ...
  if (config.require_org_approval) {
    const approvals = pathExists(
      join(runPath, "evidence-lifecycle", "disposal", "DISPOSAL_APPROVALS.json")
    )
      ? await readJson<{ quorum?: { status?: string } }>(
          join(runPath, "evidence-lifecycle", "disposal", "DISPOSAL_APPROVALS.json")
        ).catch(() => undefined)
      : undefined;
    checks.push({
...
async function writePrecheck(
  cwd: string,
  runId: string,
  checks: DisposalPrecheck["checks"]
): Promise<DisposalPrecheck> {
  const store = new RunStore(cwd);
  const ok = checks.every((check) => !check.blocking || check.ok);
  const result: DisposalPrecheck = {
    runId,
  ...
  await store.writeTextArtifact(
    runId,
    "evidence-lifecycle/disposal/DISPOSAL_PRECHECK.md",
...
function renderPrecheck(result: DisposalPrecheck): string {
