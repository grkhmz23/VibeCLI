...
export function defaultEvidenceDisposalState(): EvidenceDisposalState {
  return {
...
export function ensureEvidenceDisposalState(state: RunState): EvidenceDisposalState {
...
export async function updateEvidenceDisposalState(
  store: RunStore,
  runId: string,
  updater: (disposal: EvidenceDisposalState, state: RunState) => void
): Promise<RunState> {
