...
export type DisposalCandidates = {
  ...
  blocked: Array<{ path: string; class: string; reason: string }>;
  warnings: string[];
};
...
export type DisposalPlan = {
  ...
  preDeleteRequirements: Array<{
    name:
      ...
      | "archive_verified"
      | "ledger_verified"
      | "org_disposal_approval"
      | "disposal_attestation";
    required: boolean;
    satisfied: boolean;
    message: string;
  }>;
...
export type DisposalPrecheck = {
  ...
  createdAt: string;
  ok: boolean;
  checks: Array<{ name: string; ok: boolean; blocking: boolean; message: string }>;
  warnings: string[];
  nextActions: string[];
};
