...
export function classifyEvidencePath(path: string): EvidenceClass {
  if (path === "ledger-manifest.json" || path.includes("ledger")) return "run-ledger";
  if (path.startsWith("patches/")) return "patches";
  if (path.startsWith("rollback/")) return "rollback";
  if (path.includes("verification")) return "verification";
  if (path.includes("scanner")) return "scanner";
  if (path.startsWith("handoff/") || path.includes("HANDOFF")) return "handoff";
  ...
  if (path.startsWith("remote-attestation/")) return "remote-attestation";
...
): Pick<EvidenceInventoryFile, "sensitivity" | "excluded" | "exclusionReason" | "warnings"> {
...
export function recommendedRetention(
  evidenceClass: EvidenceClass
): "short" | "standard" | "long" | "legal_hold" {
  if (
    ["release", "provenance", "evidence", "audit", "organization", "run-ledger"].includes(
      evidenceClass
    )
  ) {
    return "long";
