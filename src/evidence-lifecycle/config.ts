...
export const defaultEvidenceLifecycleConfig: EvidenceLifecycleConfig = {
  enabled: true,
...
export async function loadEvidenceLifecycleConfig(cwd: string): Promise<EvidenceLifecycleConfig> {
  const config = await loadConfig(cwd);
  return { ...defaultEvidenceLifecycleConfig, ...config.evidence_lifecycle };
}

export async function evidenceLifecyclePaths(cwd: string): Promise<{
  archiveDir: string;
  lifecycleDir: string;
