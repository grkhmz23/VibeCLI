...
import type { EvidenceClass, EvidenceInventory, EvidenceInventoryFile } from "./types.js";
...
export async function generateEvidenceInventory(
  ...
  const inventoryFiles: EvidenceInventoryFile[] = [];
  ...
  const inventory: EvidenceInventory = {
    ...
    summary: {
      ...
      secretLikeFindings: inventoryFiles.filter((file) =>
        file.warnings.some((warning) => /secret/i.test(warning))
      ).length,
      privateKeyFindings: inventoryFiles.filter((file) =>
        /private key/i.test(file.exclusionReason ?? "")
      ).length,
      envFileFindings: inventoryFiles.filter(
        (file) => /\.env/.test(file.path) && !file.path.endsWith(".env.example")
      ).length
    },
    classes,
  ...
  await store.writeTextArtifact(
    runId,
    "evidence-lifecycle/EVIDENCE_INVENTORY.md",
...
export async function summarizeAllInventories(cwd: string): Promise<{
  runs: Array<{ runId: string; status: "generated" | "missing"; totalFiles: number | null }>;
}> {
    ...
    runs: await Promise.all(
      entries
        ...
        .map(async (entry) => {
          ...
          if (!pathExists(path))
            return { runId: entry.name, status: "missing" as const, totalFiles: null };
          const inventory = JSON.parse(await readFile(path, "utf8")) as EvidenceInventory;
          return {
            runId: entry.name,
...
function buildClasses(files: EvidenceInventoryFile[]): EvidenceInventory["classes"] {
  ...
  return classes.map((evidenceClass) => {
    ...
    return {
      ...
      requiredForRelease: ["run-ledger", "release", "provenance", "evidence"].includes(
        evidenceClass
      ),
...
function blockedWithoutRead(path: string): boolean {
  return path.endsWith(".private.pem") || /(^|\/)\.env($|\.)/.test(path);
}

function includeByDefault(evidenceClass: EvidenceClass): boolean {
