...
import type { EvidenceInventory, EvidenceLifecycleReport, LegalHoldRecord } from "./types.js";

export async function createEvidenceLifecycleReport(
  cwd: string,
  runId: string
): Promise<EvidenceLifecycleReport> {
  ...
  const retentionPlan = await readOptional<{
    policy?: string;
  ...
  const report: EvidenceLifecycleReport = {
    runId,
    ...
    inventory: {
      ...
      blockedFiles:
        inventory?.files.filter((file) => file.sensitivity === "blocked").length ??
    ...
    retentionLedger: {
      ...
      latestChainHash:
        latestRunChain?.chainHash ?? lifecycle.retentionLedger.latestChainHash ?? null
    },
    compaction: {
...
export async function createEvidenceLifecycleIndex(cwd: string): Promise<{
  createdAt: string;
  runs: Array<{
    runId: string;
    inventory: string;
  ...
  await import("node:fs/promises").then((fs) =>
    fs.writeFile(join(dir, "EVIDENCE_LIFECYCLE_INDEX.md"), renderLifecycleIndex(index), "utf8")
  );
  return index;
}
...
function renderLifecycleReport(report: EvidenceLifecycleReport): string {
...
function renderLifecycleIndex(index: { runs: Array<{ runId: string; archive: string }> }): string {
