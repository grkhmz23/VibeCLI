...
import type { EvidenceInventory, LegalHoldRecord, RetentionEnforcementPreview } from "./types.js";

export async function previewRetentionEnforcement(
  cwd: string,
  runId: string,
  ...
  const inventory = pathExists(inventoryPath)
    ? await readJson<EvidenceInventory>(inventoryPath)
  ...
  const preview: RetentionEnforcementPreview = {
    runId,
  ...
  await store.writeArtifact(
    runId,
    "evidence-lifecycle/RETENTION_ENFORCEMENT_PREVIEW.json",
    preview
  );
  await store.writeTextArtifact(
    runId,
    "evidence-lifecycle/RETENTION_ENFORCEMENT_PREVIEW.md",
    ...
    artifactHashes: [
      {
        ...
        sha256: (
          await sha256File(
            join(runPath, "evidence-lifecycle", "RETENTION_ENFORCEMENT_PREVIEW.json")
          )
        ).sha256
      }
    ]
...
async function readLegalHold(runPath: string): Promise<LegalHoldRecord | undefined> {
  const path = join(runPath, "evidence-lifecycle", "LEGAL_HOLD.json");
  return pathExists(path) ? readJson<LegalHoldRecord>(path).catch(() => undefined) : undefined;
}

function renderPreview(preview: RetentionEnforcementPreview): string {
