...
export async function verifyRetentionLedger(
  cwd: string,
  runId?: string
): Promise<{
  ok: boolean;
  ...
  for (let index = 0; index < Math.min(events.length, chain.length); index += 1) {
    ...
    const chainHash = sha256Text(
      JSON.stringify({ index, runId: event.runId, eventHash, previousChainHash })
    );
    if (entry.index !== index) errors.push(`Retention chain index ${index} is incorrect.`);
    if (entry.eventHash !== eventHash) errors.push(`Retention event ${index} hash mismatch.`);
    if (entry.previousChainHash !== previousChainHash)
      errors.push(`Retention chain ${index} previous hash mismatch.`);
    if (entry.chainHash !== chainHash) errors.push(`Retention chain ${index} hash mismatch.`);
  }
