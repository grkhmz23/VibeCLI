...
export async function proposedBranchName(
  cwd: string,
  runId: string,
  custom?: string
): Promise<string> {
  if (custom) return validateBranchName(custom);
...
export async function branchRun(
  ...
  const protectedBranch = previousBranch
    ...
    : false;
  if (protectedBranch) warnings.push(`Current branch ${previousBranch} is protected.`);
  if (dirty) warnings.push("Working tree is dirty.");
  let mode: BranchResult["mode"] = "preview";
  if (options.create) {
    ...
    if (await branchExists(cwd, branch)) {
      if (!options.switchExisting) throw new Error(`Branch ${branch} already exists.`);
