...
function subjectFromPrompt(prompt: string, style: "conventional" | "plain"): string {
...
export async function generateCommitMessage(
  cwd: string,
  runId: string,
  styleOverride?: "conventional" | "plain"
  ...
  const verificationText =
    !state.verification || state.verification.status === "not_started"
      ? "not run"
      : state.verification.status;
  const scannerText =
    !scannerStatus || scannerStatus === "not_started/not_started" ? "not run" : scannerStatus;
  const warnings = [
