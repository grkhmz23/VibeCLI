...
export async function generatePrDescription(
  ...
  const verification = await optionalJson<{ status?: string }>(
    join(runPath, "verification-results.json")
  );
  const scanners = await optionalJson<Array<{ status: string; findings?: unknown[] }>>(
    join(runPath, "scanner-results.json")
  );
  const cost = await optionalJson<{ known?: boolean; estimatedUsd?: number | null }>(
    join(runPath, "cost-estimate.json")
  );
