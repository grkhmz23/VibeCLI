...
export async function evaluateReadiness(cwd: string, runId: string): Promise<ReadinessResult> {
  ...
  if (state.scanners?.criticalFindings || state.scanners?.highFindings) {
    ...
    state.scanners?.builtinStatus === "warning" ||
  ...
  const gitStatus = await execFileAsync("git", ["status", "--short"], { cwd })
    ...
    .catch(() => "");
  if (gitStatus) warnings.push("Git working tree has local changes.");
  return {
    runId,
