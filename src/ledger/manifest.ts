...
function hashManifest(manifest: Omit<LedgerManifest, "manifestHash">): string {
  return sha256Text(JSON.stringify(manifest, null, 2));
}

export async function writeLedgerManifest(cwd: string, runId: string): Promise<LedgerManifest> {
  const store = new RunStore(cwd);
  const runPath = store.runPath(runId);
  const existing = pathExists(join(runPath, "ledger-manifest.json"))
    ? await readJson<LedgerManifest>(join(runPath, "ledger-manifest.json")).catch(() => undefined)
    : undefined;
  ...
  const manifest: LedgerManifest = { ...withoutHash, manifestHash: hashManifest(withoutHash) };
  await writeFile(join(runPath, "ledger-manifest.json"), JSON.stringify(manifest, null, 2), "utf8");
  return manifest;
}

export async function readLedgerManifest(cwd: string, runId: string): Promise<LedgerManifest> {
  return readJson<LedgerManifest>(join(new RunStore(cwd).runPath(runId), "ledger-manifest.json"));
}
