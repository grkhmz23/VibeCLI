...
import type { PatchManifestEntry } from "./patches.js";
...
async function hasFailedGate(store: RunStore, runId: string): Promise<boolean> {
...
export async function repairRun(
  ...
  if (!fixerConfig) throw new Error("Fixer Agent is not configured");
  ...
  const repairInput = {
    runId,
    prompt: state.prompt,
    verification: await readFile(
      join(store.runPath(runId), "verification-results.json"),
      "utf8"
    ).catch(() => ""),
  ...
  await writeFile(
    join(cyclePath, "repair-input.json"),
  ...
  await writeFile(
    join(cyclePath, "fixer-output.json"),
    JSON.stringify(fixerOutput, null, 2),
    "utf8"
  );
  ...
  await writeFile(
    join(cyclePath, "patch-validation.json"),
    JSON.stringify(
      { runId, cycle, ok: validation.every((entry) => entry.ok), patches: validation },
      null,
      2
    ),
    "utf8"
  ...
  await writeFile(
    join(cyclePath, "command-review.json"),
    JSON.stringify({ recommended: collectCommandRecommendations({ fixer: fixerOutput }) }, null, 2),
    "utf8"
  );
  const manifest = await readPatchManifest(store, runId);
  const entries: PatchManifestEntry[] = fixerOutput.patches.map((patch) => ({
    agent: "fixer",
    path: patch.path,
