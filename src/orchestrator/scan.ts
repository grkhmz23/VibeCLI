...
export async function scanRunBuiltin(cwd: string, runId: string): Promise<ScannerResult[]> {
...
async function scannerAvailable(binary: string): Promise<boolean> {
  try {
...
export async function scanRunExternal(
  cwd: string,
  runId: string,
  confirm?: string
): Promise<ExternalScannerResults> {
  ...
  const entries: ExternalScannerResults["scanners"] = [];
  ...
  const status: ExternalScannerResults["status"] = entries.some((entry) => entry.status === "fail")
    ? "fail"
    : entries.some((entry) => entry.status === "warning")
      ...
      : entries.every((entry) => entry.status === "skipped")
        ? "skipped"
        : "pass";
  const result: ExternalScannerResults = {
    runId,
    startedAt,
