...
import type { ModelUsageEntry } from "../cost/usage.js";
...
import type { AgentState, GateState, RunEvent, RunState } from "./state.js";
...
export function createInitialState(args: {
  ...
  const timestamp = now();
  const agents = agentRoleIds.reduce(
    (accumulator, id) => ({
      ...accumulator,
      [id]: { id, status: "queued", artifacts: [] }
    }),
    {} as Record<AgentRoleId, AgentState>
  );
  const gates = agentRoleIds.reduce(
    (accumulator, id) => ({
      ...accumulator,
      [id]: { agent: id, status: "not_started", condition: agentRoles[id].gateCondition }
    }),
    {} as Record<AgentRoleId, GateState>
  );
  return {
    runId: args.runId,
    prompt: args.prompt,
...
function event(agent: AgentRoleId | undefined, type: string, message: string): RunEvent {
  return { timestamp: now(), agent, type, message };
}

function deterministicAgentArtifact(agent: AgentRoleId, state: RunState): object {
  return {
    runId: state.runId,
    agent,
    phase: 1,
...
export async function executePhaseOneWorkflow(args: {
  ...
  if (args.config) state.routingStrategy = routingStrategyForProfile(args.config, args.profile);
...
function usageEntry(response: AgentRunResponse): ModelUsageEntry {
  return {
    agent: response.role,
...
function providerEnvErrors(config: VibeConfig, profile: string): string[] {
  const agentConfig = config.profiles[profile]?.agents;
  if (!agentConfig) return [`Profile ${profile} is not configured`];
...
export async function executePhaseTwoLiveWorkflow(args: {
  ...
  for (const agent of liveAgents) {
    ...
    if (!provider) throw new Error(`Provider ${route.selected.provider} is not configured`);
    ...
    if (result.response) usageEntries.push(usageEntry(result.response));
    const budgetReport = await writeBudgetReport({
      cwd: args.cwd,
      runId,
      config: args.config,
    ...
    if (result.ok) {
      ...
      await store.writeArtifact(
        runId,
        `agent-outputs/${agent}.validation-error.json`,
        result.validation
      );
  ...
  const securityVerdict =
    typeof securityOutput === "object" && securityOutput !== null && "verdict" in securityOutput
