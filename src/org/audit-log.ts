...
export async function appendOrgAuditEvent(
  cwd: string,
  event: Omit<OrgAuditEvent, "version" | "createdAt">
): Promise<OrgAuditChainEntry> {
  const paths = await orgPaths(cwd);
  await ensureDir(paths.auditLogDir);
  const createdAt = new Date().toISOString();
  const fullEvent: OrgAuditEvent = {
    version: 1,
    createdAt,
...
export async function readOrgAudit(cwd: string): Promise<{
  ...
  const events = pathExists(logPath)
    ? (await readFile(logPath, "utf8"))
        .split(/\r?\n/)
        .filter(Boolean)
        .map((line) => JSON.parse(line) as OrgAuditEvent)
    : [];
  const chain = pathExists(join(paths.auditLogDir, "audit-chain.json"))
    ? await readJson<OrgAuditChainEntry[]>(join(paths.auditLogDir, "audit-chain.json")).catch(
        () => []
      )
    : [];
  return { events, chain };
...
export async function verifyOrgAuditLog(cwd: string): Promise<{
  ok: boolean;
  eventCount: number;
  ...
  const errors: string[] = [];
  if (events.length !== chain.length) errors.push("Audit event count does not match chain length.");
  for (let index = 0; index < Math.min(events.length, chain.length); index += 1) {
    ...
    if (entry.index !== index) errors.push(`Audit chain index ${index} is incorrect.`);
    if (entry.eventHash !== eventHash) errors.push(`Audit event ${index} hash mismatch.`);
    if (entry.previousChainHash !== previousChainHash)
      errors.push(`Audit chain ${index} previous hash mismatch.`);
    if (entry.chainHash !== expectedChainHash) errors.push(`Audit chain ${index} hash mismatch.`);
  }
  return {
    ok: errors.length === 0,
