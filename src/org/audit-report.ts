...
import type { EvidenceExportManifest, OrgAuditReport } from "./types.js";

export async function createOrgAuditReport(cwd: string, runId: string): Promise<OrgAuditReport> {
  ...
  const evidenceManifest = pathExists(evidenceManifestPath)
    ? await readJson<EvidenceExportManifest>(evidenceManifestPath).catch(() => undefined)
    : undefined;
...
function renderOrgReport(report: OrgAuditReport): string {
