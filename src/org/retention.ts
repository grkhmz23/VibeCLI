...
const evidenceClasses: RetentionPlan["evidenceClasses"][number]["class"][] = [
  "run-ledger",
  "release",
  "handoff",
  "provenance",
...
export async function createRetentionPlan(
  cwd: string,
  runId: string,
  options: { policy?: string; mark?: boolean; confirm?: string; purgePreview?: boolean } = {}
  ...
  if (!policy) throw new Error(`Retention policy ${policyName} is not configured.`);
  ...
  const classes = await Promise.all(
    evidenceClasses.map(async (evidenceClass) => ({
      class: evidenceClass,
...
export async function readRetentionPlan(cwd: string, runId: string): Promise<RetentionPlan | null> {
  const path = join(new RunStore(cwd).runPath(runId), "org", "RETENTION_PLAN.json");
  return pathExists(path) ? readJson<RetentionPlan>(path) : null;
}

async function classPaths(root: string, evidenceClass: string): Promise<string[]> {
  ...
  for (const item of mapping[evidenceClass] ?? []) {
    ...
    if (item.endsWith(".json")) result.push(item);
    else result.push(...(await collectRelative(fullPath, item)));
  }
  return result;
}

async function collectRelative(dir: string, prefix: string): Promise<string[]> {
  const entries = await readdir(dir, { withFileTypes: true }).catch(() => []);
  const files: string[] = [];
  for (const entry of entries) {
    const rel = join(prefix, entry.name).replace(/\\/g, "/");
    const fullPath = join(dir, entry.name);
    if (entry.isDirectory()) files.push(...(await collectRelative(fullPath, rel)));
    if (entry.isFile()) files.push(rel);
  }
  return files;
}

function renderRetentionPlan(plan: RetentionPlan): string {
