...
export async function orgStatus(cwd: string): Promise<{
  enabled: boolean;
  orgId: string;
  orgName: string;
  ...
  auditLog: { events: number; latestChainHash: string | null };
  key: { status: "missing" | "present"; publicKeyFingerprint: string | null };
}> {
  ...
  for (const reviewer of config.organization.reviewers) {
    for (const role of reviewer.roles) roles[role] = (roles[role] ?? 0) + 1;
  }
  return {
...
export async function initOrganization(
  cwd: string,
  options: { confirm?: string; noEnable?: boolean; force?: boolean; createKey?: boolean }
  ...
  if (config.organization.enabled && !options.force) {
    throw new Error(
      "Organization workflow is already enabled; use --force with exact confirmation."
    );
  }
  ...
  await writeFile(
    join(paths.orgRoot, "ORG_STATUS.md"),
    renderOrgStatus(await orgStatus(cwd)),
    "utf8"
  );
...
export async function listOrgReviewers(
  cwd: string
): Promise<Array<{ id: string; displayName: string; roles: string[] }>> {
  const config = await loadConfig(cwd);
...
export async function orgAuditSummary(
  cwd: string,
  verify = false
): Promise<{
  ok?: boolean;
...
function renderOrgStatus(status: Awaited<ReturnType<typeof orgStatus>>): string {
