...
export function applyParsedPatchToContent(original: string, patch: ParsedFilePatch): string {
...
export async function applyParsedFilePatch(
  repoRoot: string,
  ...
  if (!pathExists(fullPath)) throw new Error(`Cannot modify missing file: ${patch.path}`);
  await writeFile(
    fullPath,
    applyParsedPatchToContent(await readFile(fullPath, "utf8"), patch),
    "utf8"
  );
}
