...
function profile(name: string, overrides: Partial<PolicyProfile> = {}): PolicyProfile {
  const base: PolicyProfile = {
    version: 1,
    name,
...
export const defaultPolicyProfiles: Record<string, PolicyProfile> = {
  fast: profile("fast", {
  ...
  "strict-enterprise": profile("strict-enterprise", {
    description:
      "Strict enterprise policy with exact confirmations and external scanner expectations",
    routing: { strategy: "strongest", require_fallbacks_for_live_agents: true },
    scanner: { require_builtin_scanners: true, require_external_scanners: true },
    approval: {
      require_exact_confirmation: true,
