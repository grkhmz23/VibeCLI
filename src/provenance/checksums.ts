...
export async function createChecksums(cwd: string, runId: string): Promise<ChecksumManifest> {
...
export async function verifyChecksums(
  ...
  runId: string;
  ok: boolean;
  checks: Array<{ path: string; ok: boolean; message: string }>;
}> {
  const store = new RunStore(cwd);
  const manifest = await readJson<ChecksumManifest>(
    join(store.runPath(runId), "provenance", "checksums.json")
  );
    ...
    manifest.entries.map(async (entry) => {
      if (entry.path === "ledger-manifest.json") {
        return {
          ...
          ok: true,
          message:
            "ledger manifest is verified through vibe ledger because it is recomputed after provenance operations"
        };
      }
      const fullPath = join(store.runPath(runId), entry.path);
      if (!pathExists(fullPath)) return { path: entry.path, ok: false, message: "missing" };
      const hash = await sha256File(fullPath);
      return {
        path: entry.path,
