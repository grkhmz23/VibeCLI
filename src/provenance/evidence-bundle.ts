...
export async function createEvidenceBundle(
  ...
  if (options.sign) {
    ...
    if (!verified?.ok) await signProvenance(cwd, runId, { confirm: `SIGN PROVENANCE ${runId}` });
  }
  ...
  await store.writeTextArtifact(
    ...
    renderEvidence(files, options.sign ?? false)
  );
...
export async function verifyEvidenceBundle(
  ...
  const store = new RunStore(cwd);
  const manifest = await readJson<EvidenceManifest>(
    join(store.runPath(runId), "evidence", "EVIDENCE_MANIFEST.json")
  );
...
function renderEvidence(files: Array<{ path: string }>, signed: boolean): string {
...
function sanitizeEvidenceContent(file: string, content: string): string {
  if (file === "ledger-manifest.json") {
    try {
      const parsed = JSON.parse(content) as { entries?: Array<{ path?: string }> };
      parsed.entries = (parsed.entries ?? []).filter((entry) => {
        const entryPath = entry.path ?? "";
        return (
          !entryPath.includes("agent-outputs") &&
