...
import { readApprovalNotes, renderApprovalNotes, type ApprovalNote } from "../approvals/notes.js";
...
export async function addReleaseApproval(
  cwd: string,
  runId: string,
  args: {
  ...
  if (!args.decision && !args.reviewer && !args.note) return readApprovalNotes(cwd, runId);
  ...
  const approval: ApprovalNote = { ...unsigned, signature: localPayloadSignature(unsigned) };
  ...
  await store.writeTextArtifact(
    runId,
    "release/RELEASE_APPROVALS.md",
    renderApprovalNotes(notes.filter((note) => note.type === "release"))
  );
