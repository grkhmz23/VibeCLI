...
import type { CiStatus, ReleaseRisk, ReleaseSummary, VersionPlan } from "./types.js";
...
function verificationStatus(value: string | undefined): ReleaseSummary["gates"]["verification"] {
...
function scannerStatus(
  state: Awaited<ReturnType<RunStore["readState"]>>
): ReleaseSummary["gates"]["scanners"] {
  const scanners = state.scanners;
  if (!scanners || scanners.builtinStatus === "not_started") return "not_started";
  if (
    scanners.criticalFindings > 0 ||
    scanners.highFindings > 0 ||
    scanners.builtinStatus === "failed"
  )
    return "failed";
  if (scanners.builtinStatus === "warning") return "warning";
  if (scanners.builtinStatus === "skipped") return "skipped";
  return "passed";
}
...
export async function buildReleaseSummary(
  cwd: string,
  runId: string,
  options: { channel?: string; strict?: boolean } = {}
  ...
  const version = await optionalJson<VersionPlan>(join(runPath, "release", "version-plan.json"));
  ...
  if (!ledger) blockingReasons.push("Ledger manifest is missing.");
  else if (!ledger.ok) blockingReasons.push("Ledger verification failed.");
  if (state.verification?.status !== "passed")
    warnings.push("Tests/build/lint were not run or did not pass.");
  if (scannerStatus(state) === "not_started") warnings.push("Scanner status was not run.");
  if (!ci) warnings.push("CI status was not ingested.");
  ...
  const unresolvedCritical = exceptions.exceptions.filter(
    (item) =>
  ...
  const verdict =
    blockingReasons.length > 0
      ...
      : warnings.length > 0
        ? "ready_with_warnings"
        : (`ready_for_${channel}` as ReleaseSummary["releaseVerdict"]);
...
export async function generateReleasePacket(
  ...
  const packet = renderReleasePacket(
    summary,
    state.prompt,
    state.provenance,
  ...
  await store.writeTextArtifact(
    ...
    renderReleaseChecklist(summary)
  );
...
export function renderReleasePacket(
  ...
  prompt: string,
  provenance?: {
    statement?: { status: string };
    signature?: { status: string };
    evidence?: { status: string };
    githubReleaseDraft?: { status: string };
  },
  remoteAttestation?: {
    export?: { status: string };
    transparency?: { status: string };
    submission?: { status: string };
    registryMetadata?: { status: string };
  },
  organization?: {
    enabled: boolean;
    policyBundle?: { status: string };
    approvals?: { status: string; approvedCount?: number; missingRoles?: string[] };
    receiptRefresh?: { status: string };
    retention?: { status: string };
    evidenceExport?: { status: string };
    audit?: { status: string };
  },
  audit?: {
    schema?: { activeSchema?: string };
    coverage?: {
      status: string;
    ...
    export?: { status: string };
    complianceBundle?: { status: string };
    auditorHandoff?: { status: string };
  },
  evidenceLifecycle?: {
    inventory?: { status: string; totalFiles?: number };
    archive?: { status: string; archivePath?: string };
    retentionLedger?: { status: string };
    legalHold?: { status: string };
...
function renderReleaseChecklist(summary: ReleaseSummary): string {
...
function renderReleaseNotes(summary: ReleaseSummary): string {
