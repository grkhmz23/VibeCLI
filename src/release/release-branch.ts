...
async function dirty(cwd: string): Promise<boolean> {
  return Boolean(await git(["status", "--short"], cwd));
}

export async function releaseBranchRun(
  cwd: string,
  runId: string,
  options: {
    create?: boolean;
  ...
  const result: ReleaseBranchResult = {
    runId,
  ...
  if (options.confirm !== `CREATE RELEASE BRANCH ${runId}`) {
    throw new Error(
      `Release branch creation requires exact confirmation: CREATE RELEASE BRANCH ${runId}`
    );
  }
