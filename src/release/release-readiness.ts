...
export async function evaluateReleaseReadiness(
  ...
  const summary = await readJson<ReleaseSummary>(
    join(store.runPath(runId), "release", "RELEASE_SUMMARY.json")
  ).catch(() => undefined);
  const deployment = await readJson<DeploymentReadiness>(
    join(store.runPath(runId), "release", "deployment-readiness.json")
  ).catch(() => undefined);
  ...
  const passed: string[] = [];
  if (!summary) blockingReasons.push("Release packet is missing.");
  ...
  else if (channel === "production") blockingReasons.push("Production requires verification pass.");
  else warnings.push("Verification is not passed.");
  if (ci?.status === "passed") passed.push("CI passed");
  else if (channel === "production") blockingReasons.push("Production requires CI pass.");
  else warnings.push("CI is not passed or not ingested.");
  ...
  const releaseApproved = notes.some(
    (note) => note.type === "release" && note.decision === "approved"
  );
  ...
  const provenanceRequired =
    policy?.provenance.require_signed_provenance ||
  ...
  if (policy?.provenance.require_provenance_statement) {
    if (!pathExists(join(store.runPath(runId), "provenance", "provenance-statement.json"))) {
      ...
      else warnings.push("Provenance statement is missing.");
    } else {
  ...
  if (policy?.remote_attestation.require_transparency_entry_for_submission) {
    ...
    if (transparencyOk) passed.push("transparency entry present");
    else if (channel === "production")
      blockingReasons.push("Production requires transparency entry.");
    else warnings.push("Transparency entry is missing.");
  }
  if (policy?.registry_metadata.require_registry_metadata_for_release) {
    ...
    if (registryOk) passed.push("registry metadata generated");
    else if (channel === "production") {
  ...
  if (policy?.audit.require_signed_audit_export_for_production) {
    const auditSigned =
      state.audit?.export.status === "signed" || state.audit?.export.status === "verified";
    if (auditSigned) passed.push("signed audit export present");
    else if (channel === "production") {
  ...
  const orgApprovalRequired =
    config.organization.require_multi_reviewer_approval_for_release ||
...
export function renderReleaseReadiness(value: ReleaseReadiness): string {
