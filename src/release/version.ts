...
function compareSemver(a: string, b: string): number {
  ...
  if (!left || !right) throw new Error("Cannot compare invalid semver");
...
function bumpVersion(current: string, bump: VersionBump, preid: string | null): string {
  ...
  if (bump === "prerelease") return `${major}.${minor}.${patch + 1}-${preid ?? "beta"}.0`;
  return current;
}

function inferBump(prompt: string): VersionBump {
  const value = prompt.toLowerCase();
  if (/\bbreaking\b|major/.test(value)) return "major";
  if (/\bsecurity\b|\bfix\b|bug/.test(value)) return "patch";
  if (/\badd\b|\bfeature\b|\bnew\b/.test(value)) return "minor";
  return "patch";
}

async function detectedVersionFiles(
  cwd: string
): Promise<Array<{ path: string; currentVersion: string }>> {
  const files: Array<{ path: string; currentVersion: string }> = [];
  ...
  for (const path of ["pyproject.toml", "Cargo.toml"]) {
    ...
    if (match?.[1]) files.push({ path, currentVersion: match[1] });
  }
  return files;
}
...
export async function planVersion(
  ...
  if (options.version && currentVersion && options.version === currentVersion) bump = "none";
...
export function renderVersionPlan(plan: VersionPlan): string {
  return `# Version Plan
