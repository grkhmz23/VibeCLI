...
import type { RemotePayload, RemoteSubmissionReceipt } from "./types.js";
...
export async function submitAttestation(
  ...
  if (!options.dryRun && options.confirm !== `SUBMIT ATTESTATION ${runId} TO ${targetName}`) {
    throw new Error(
      `Remote attestation submission requires exact confirmation: SUBMIT ATTESTATION ${runId} TO ${targetName}`
    );
  }
  ...
  const payload = await readJson<RemotePayload>(
    join(store.runPath(runId), "remote-attestation", "REMOTE_PAYLOAD.json")
  );
  const requestText = JSON.stringify(payload, null, 2);
  const warnings: string[] = [];
  const errors: string[] = [];
  if (!targetName) errors.push("Target is required.");
  if (!target) errors.push(`Remote attestation target ${targetName || "unknown"} is missing.`);
  if (target && !target.enabled)
    errors.push(`Remote attestation target ${targetName} is disabled.`);
  if (
    !config.remote_attestation.allow_remote_submission &&
  ...
  const signedRequired =
    config.remote_attestation.require_signed_provenance ||
  ...
  const evidenceRequired =
    config.remote_attestation.require_evidence_bundle ||
  ...
  if (
    policy?.remote_attestation.require_transparency_entry_for_submission &&
  ...
  if (
    (config.remote_attestation.require_ledger_pass ||
  ...
  if (options.dryRun) {
    ...
    if (errors.length === 0) {
      ...
      if (!submitted) errors.push(`Remote target returned HTTP ${response.statusCode}.`);
    }
  }
  const receipt: RemoteSubmissionReceipt = {
    runId,
  ...
  await store.writeTextArtifact(
    runId,
    "remote-attestation/REMOTE_SUBMISSION.md",
...
function renderReceipt(receipt: RemoteSubmissionReceipt): string {
