...
import type { RemoteAttestationTarget } from "./types.js";
...
export async function listRemoteTargets(cwd: string): Promise<TargetSummary[]> {
  const config = await loadConfig(cwd);
  return Object.entries(config.remote_attestation.targets).map(([name, target]) =>
    summarizeTarget(name, target)
  );
}

export async function doctorRemoteTargets(
  cwd: string,
  options: { ping?: boolean; confirm?: string } = {}
  ...
  for (const [name, target] of Object.entries(config.remote_attestation.targets)) {
    ...
    if (target.token_env && !envPresent) warnings.push(`Token env ${target.token_env} is not set.`);
    ...
    if (options.ping && errors.length === 0) {
      ...
      if (!response) warnings.push("Ping failed.");
    }
    results.push({
      ...summary,
      ok: errors.length === 0,
...
export async function addRemoteTarget(
  ...
  if (args.confirm !== `ADD REMOTE TARGET ${args.name}`) {
    throw new Error(
      `Remote target add requires exact confirmation: ADD REMOTE TARGET ${args.name}`
    );
  }
  const config = await loadConfig(cwd);
  if (config.remote_attestation.targets[args.name] && !args.replace) {
    throw new Error(
      `Remote target ${args.name} already exists; use --replace with exact confirmation.`
    );
  }
  const target: RemoteAttestationTarget = {
    type: "generic-http",
    url: args.url,
...
export async function disableRemoteTarget(
  cwd: string,
  name: string,
  confirm?: string
): Promise<TargetSummary> {
  if (confirm !== `DISABLE REMOTE TARGET ${name}`) {
    throw new Error(
      `Remote target disable requires exact confirmation: DISABLE REMOTE TARGET ${name}`
    );
  }
  const config = await loadConfig(cwd);
  const target = config.remote_attestation.targets[name];
  if (!target) throw new Error(`Remote target ${name} does not exist.`);
  target.enabled = false;
  await saveConfig(cwd, config);
  return summarizeTarget(name, target);
}

export async function removeRemoteTarget(
  cwd: string,
  name: string,
  confirm?: string
  ...
  if (confirm !== `REMOVE REMOTE TARGET ${name}`) {
    throw new Error(
      `Remote target removal requires exact confirmation: REMOVE REMOTE TARGET ${name}`
    );
  }
  const config = await loadConfig(cwd);
...
function summarizeTarget(name: string, target: RemoteAttestationTarget): TargetSummary {
  return {
    name,
    type: target.type,
...
function redactedHeaders(target: RemoteAttestationTarget): Record<string, string> {
  return Object.fromEntries(
    Object.entries(target.headers ?? {}).filter(([name]) => !/^authorization$/i.test(name))
  );
}
