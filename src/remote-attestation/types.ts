...
export type AttestationExport = {
  ...
  git: { branch: string | null; commitSha: string | null; repositoryRemote: string | null };
  provenance: {
  ...
  evidence: { archiveSha256: string | null; manifestSha256: string | null; signed: boolean };
  ledger: { manifestSha256: string | null; verified: boolean };
  verification: { status: string | null };
  scanners: { status: string | null; criticalFindings: number; highFindings: number };
  ci: { status: string | null };
  policy: { name: string | null; exceptions: { open: number; approved: number; rejected: number } };
...
export type TransparencyEntry = {
  ...
  createdAt: string;
  entryHash: string;
  release: { channel: string | null; version: string | null; tag: string | null };
  subjects: Array<{ name: string; sha256: string }>;
  provenance: {
    signed: boolean;
    envelopeSha256: string | null;
    publicKeyFingerprint: string | null;
  ...
  ledger: { manifestSha256: string | null; verified: boolean };
  policy: { name: string | null; strict: boolean };
  attestations: {
