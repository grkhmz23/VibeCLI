...
export function validateRemoteTarget(
  name: string,
  target: RemoteAttestationTarget,
  config: Pick<RemoteAttestationConfig, "allow_localhost_targets" | "require_https_targets">
): string[] {
...
export function validateRemoteAttestationConfig(config: RemoteAttestationConfig): string[] {
  const errors: string[] = [];
  ...
  if (config.include_evidence_archive_by_default) {
    errors.push(
      "remote_attestation.include_evidence_archive_by_default must remain false in Phase 11"
    );
  }
