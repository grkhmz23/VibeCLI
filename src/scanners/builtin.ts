...
function result(scanner: string, findings: ScannerResult["findings"]): ScannerResult {
  if (findings.some((finding) => finding.severity === "critical" || finding.severity === "high")) {
    return { scanner, status: "fail", findings };
  }
  if (findings.length > 0) return { scanner, status: "warning", findings };
  return { scanner, status: "pass", findings: [] };
}

export function protectedFileTouchScanner(context: ScannerContext): Promise<ScannerResult> {
  return Promise.resolve(
    result(
      "protected-file-touch",
      context.filesChanged.filter(isProtectedPath).map((file) => ({
        severity: "critical",
        file,
...
export async function secretPatternScanner(context: ScannerContext): Promise<ScannerResult> {
  const findings: ScannerResult["findings"] = [];
  for (const file of context.filesChanged) {
    const fullPath = join(context.repoRoot, file);
    if (!pathExists(fullPath) || isProtectedPath(file)) continue;
...
export async function envExampleScanner(context: ScannerContext): Promise<ScannerResult> {
...
export async function packageScriptScanner(context: ScannerContext): Promise<ScannerResult> {
  ...
  return result(
    "package-script",
    ["test", "build", "lint"].some((name) => scripts[name])
      ? []
      : [
          {
            severity: "low",
...
export function testPresenceScanner(context: ScannerContext): Promise<ScannerResult> {
  const changedTests = context.filesChanged.some(
    (file) => /(\.|\/)(test|spec)\./.test(file) || file.includes("__tests__/")
  );
  return Promise.resolve(
    result(
      "test-presence",
