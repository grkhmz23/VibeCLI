...
import {
  installDefaultAuditSchemas,
  listAuditSchemas,
  loadAuditSchema,
  validateAuditSchemas
} from "../audit/schema-loader.js";
...
import { parseConsoleCommand, type ConsoleCommand } from "./shortcuts.js";
...
async function history(cwd: string, entry: object): Promise<void> {
...
async function startupHeader(context: ConsoleContext): Promise<string> {
  ...
  const providers = Object.entries(config.providers).map(([name, provider]) => ({
    name,
    status:
      "api_key_env" in provider && !process.env[provider.api_key_env]
...
async function statusText(context: ConsoleContext, runId?: string): Promise<string> {
  const store = new RunStore(context.cwd);
  const id = runId ?? (await store.latestRunId());
  if (!id) return "No runs found.";
  return renderRunSummary(await store.readState(id), context.theme);
}

export async function watchRunSnapshots(
  context: ConsoleContext,
  runId?: string,
  options: { iterations?: number; intervalMs?: number } = {}
): Promise<string[]> {
  const store = new RunStore(context.cwd);
  const id = runId ?? (await store.latestRunId());
  if (!id) return ["No runs found."];
...
async function watchRun(context: ConsoleContext, runId?: string): Promise<string> {
...
export async function executeConsoleCommand(
  ...
  switch (command.type) {
    ...
    case "mode":
      if (!command.mode) return `Current mode: ${context.mode}`;
    ...
    case "stream":
      if (command.enabled === undefined) return `Streaming mode: ${context.stream ? "on" : "off"}`;
    ...
    case "policy":
      if (!command.name) return `Current policy: ${context.policy ?? "company-grade"}`;
    ...
    case "apply":
      ...
      return JSON.stringify(
        await applyRun(context.cwd, command.runId, { confirm: command.confirm }),
        null,
        2
      );
    case "rollback":
      ...
      return JSON.stringify(
        await rollbackRun(context.cwd, command.runId, { confirm: command.confirm }),
        null,
        2
      );
    case "providers": {
    ...
    case "verify":
      ...
      return JSON.stringify(
        await verifyRun(context.cwd, command.runId, { confirm: command.confirm }),
        null,
        2
      );
    case "scan":
      return JSON.stringify(
        command.external
          ...
          : await scanRunBuiltin(context.cwd, command.runId),
        null,
        2
      );
    case "repair":
      return JSON.stringify(
        await repairRun(context.cwd, command.runId, {
          live: command.live,
    ...
    case "ledger":
      return JSON.stringify(
        command.verify
          ...
          : await writeLedgerManifest(context.cwd, command.runId),
        null,
        2
      );
    case "handoff":
      return JSON.stringify(
        command.verify
          ...
          : await createHandoffBundle(context.cwd, command.runId),
        null,
        2
      );
    case "pr-body":
    ...
    case "approvals":
      return JSON.stringify(
        command.verify
          ...
          : await readApprovalNotes(context.cwd, command.runId),
        null,
        2
      );
    case "github":
      ...
      if (command.mode && command.mode !== "summary") {
        ...
        return JSON.stringify(
          await updateGithubPr(context.cwd, command.runId ?? "", {
            pr: command.pr ?? "",
            mode: command.mode,
            confirm: command.confirm
          }),
    ...
    case "github-release":
      ...
      return JSON.stringify(
        await githubReleaseDraft(context.cwd, command.runId, {
    ...
    case "branch":
      ...
      return JSON.stringify(
        await branchRun(context.cwd, command.runId, {
    ...
    case "commit":
      ...
      return JSON.stringify(
        await commitRun(context.cwd, command.runId, {
          create: command.create,
          confirm: command.confirm
        }),
        null,
    ...
    case "feedback": {
      if (command.github) {
        ...
        return JSON.stringify(
          await ingestGithubFeedback(context.cwd, command.runId, command.pr ?? "", command.confirm),
          null,
          2
        );
      }
      if (command.file) {
        return JSON.stringify(
          await ingestLocalFeedback(context.cwd, command.runId, command.file),
          null,
          2
        );
      }
      ...
      return pathExists(path)
        ? JSON.stringify(await readJson<ReviewerFeedback>(path), null, 2)
    ...
    case "merge-readiness":
      ...
      return JSON.stringify(
        await evaluateMergeReadiness(context.cwd, command.runId, {
    ...
    case "changelog":
      ...
      return JSON.stringify(
        command.write
          ...
          : await generateChangelogEntry(context.cwd, command.runId),
        null,
        2
      );
    case "version":
    ...
    case "release-branch":
      ...
      return JSON.stringify(
        await releaseBranchRun(context.cwd, command.runId, {
          create: command.create,
          confirm: command.confirm,
    ...
    case "tag":
      ...
      return JSON.stringify(
        await tagRun(context.cwd, command.runId, {
          create: command.create,
      ...
      return JSON.stringify(
        command.github
          ...
          : command.file
            ...
            : await showOrCreateLocalCiStatus(context.cwd, command.runId),
        null,
        2
      );
    case "deployment-readiness":
      return JSON.stringify(
        await evaluateDeploymentReadiness(context.cwd, command.runId, {
          channel: command.channel
        }),
        null,
        2
    ...
    case "release-approval":
      if (
        (command.decision || command.reviewer || command.note) &&
      ...
      return JSON.stringify(
        await addReleaseApproval(context.cwd, command.runId, {
    ...
    case "release-readiness":
      return JSON.stringify(
        await evaluateReleaseReadiness(context.cwd, command.runId, {
          channel: command.channel
        }),
        null,
        2
    ...
    case "provenance":
      ...
      if (command.keyCommand === "init") {
        if (
          command.confirm !== (command.rotate ? "ROTATE PROVENANCE KEY" : "CREATE PROVENANCE KEY")
        ) {
          return `Provenance key init requires exact confirmation: /provenance key init --confirm "CREATE PROVENANCE KEY"`;
        }
        return JSON.stringify(
          await initProvenanceKey(context.cwd, {
            confirm: command.confirm,
      ...
      if (!command.runId) return "Usage: /provenance <run-id>";
      ...
      return JSON.stringify(
        command.verify
          ...
          : command.sign
            ...
            : await generateProvenanceStatement(context.cwd, command.runId),
        null,
        2
      );
    case "checksums":
      return JSON.stringify(
        command.verify
          ...
          : await createChecksums(context.cwd, command.runId),
        null,
        2
      );
    case "evidence":
    ...
    case "remote-targets":
      ...
      if (command.command === "doctor") {
        ...
        return JSON.stringify(
          await doctorRemoteTargets(context.cwd, {
            ping: command.ping,
            confirm: command.confirm
          }),
          null,
      ...
      if (command.command === "add") {
        ...
        return JSON.stringify(
          await addRemoteTarget(context.cwd, {
            name: command.name,
            url: command.url,
      ...
      if (!command.name) return "Usage: /remote-targets disable <name>";
      if (command.command === "disable") {
        ...
        return JSON.stringify(
          await disableRemoteTarget(context.cwd, command.name, command.confirm),
          null,
          2
        );
      }
      ...
      return JSON.stringify(
        await removeRemoteTarget(context.cwd, command.name, command.confirm),
        null,
        2
      );
    case "attestation":
      ...
      if (command.command === "receipt") {
        return JSON.stringify(
          await readRemoteSubmissionReceipt(context.cwd, command.runId),
          null,
          2
        );
      }
      ...
      return JSON.stringify(
        await submitAttestation(context.cwd, command.runId, {
    ...
    case "transparency":
      ...
      if (!command.runId) return "Usage: /transparency <run-id>";
      if (command.command === "append") {
        ...
        return JSON.stringify(
          await appendTransparencyEntry(context.cwd, command.runId, command.confirm),
          null,
          2
        );
      }
    ...
    case "registry-metadata":
      return JSON.stringify(
        await generateRegistryMetadata(context.cwd, command.runId, {
          image: command.image,
          tag: command.tag
        }),
        null,
    ...
    case "org":
      ...
      if (command.command === "init") {
        ...
        return JSON.stringify(
          await initOrganization(context.cwd, {
            confirm: command.confirm,
      ...
      if (command.keyCommand === "init") {
        ...
        return JSON.stringify(
          await initOrgKey(context.cwd, { confirm: command.confirm, rotate: command.rotate }),
          null,
          2
        );
      }
    ...
    case "org-policy":
      ...
      return JSON.stringify(
        await createOrgPolicyBundle(context.cwd, {
          sign: command.sign,
          confirm: command.confirm
        }),
        null,
      ...
      return JSON.stringify(
        command.verify
          ...
          : command.add
            ...
            : await getApprovalMatrix(context.cwd, command.runId),
        null,
        2
      );
    case "receipt-refresh":
      ...
      return JSON.stringify(
        await refreshReceipt(context.cwd, command.runId, {
          dryRun: command.dryRun,
    ...
    case "retention":
      ...
      return JSON.stringify(
        await createRetentionPlan(context.cwd, command.runId, {
      ...
      return JSON.stringify(
        command.verify
          ...
          : await createEvidenceExport(context.cwd, command.runId, {
              mode: command.mode as "minimal" | "audit" | "forensic-redacted" | undefined
            }),
        null,
        2
      );
    ...
    case "audit-schemas":
      ...
      if (command.command === "show") {
        if (!command.name) return "Usage: /audit-schemas show <name>";
        return JSON.stringify(
          auditSchemaSummary(await loadAuditSchema(context.cwd, command.name)),
          null,
          2
        );
      }
      ...
      return JSON.stringify(
        await installDefaultAuditSchemas(context.cwd, {
          force: command.force,
          confirm: command.confirm
        }),
        null,
    ...
    case "audit-map":
      return JSON.stringify(
        await generateAuditEvidenceMap(context.cwd, command.runId, { schema: command.schema }),
        null,
        2
      );
    case "audit-coverage":
      return JSON.stringify(
        await generateAuditCoverage(context.cwd, command.runId, { schema: command.schema }),
        null,
        2
      );
    case "audit-gaps":
      return JSON.stringify(
        await generateAuditGaps(context.cwd, command.runId, { schema: command.schema }),
        null,
        2
      );
    case "audit-export":
    ...
    case "reviewer-directory":
      ...
      if (!command.file) return "Usage: /reviewer-directory import --file <path>";
      if (command.command === "validate") {
    ...
    case "evidence-inventory":
      ...
      if (!command.runId) return "Usage: /evidence-inventory <run-id> | --all";
      return JSON.stringify(
        await generateEvidenceInventory(context.cwd, command.runId, { deep: command.deep }),
        null,
        2
      );
    case "evidence-lifecycle":
      ...
      if (!command.runId) return "Usage: /evidence-lifecycle <run-id> | --all";
      return JSON.stringify(
        await createEvidenceLifecycleReport(context.cwd, command.runId),
        null,
        2
      );
    case "retention-enforce":
      return JSON.stringify(
        await previewRetentionEnforcement(context.cwd, command.runId, {
          policy: command.policy
        }),
        null,
        2
    ...
    case "retention-ledger":
      if (command.record) {
        if (!command.runId) return "Retention ledger manual record requires a run id.";
        ...
        return JSON.stringify(
          await recordManualRetentionEvent(context.cwd, command.runId, {
            event: command.event as never,
      ...
      return JSON.stringify(
        command.verify
          ...
          : await retentionLedgerSummary(context.cwd, command.runId),
        null,
        2
      );
    case "legal-hold":
      ...
      return JSON.stringify(
        command.enable
          ...
          : command.release
            ...
            : await legalHoldStatus(context.cwd, command.runId),
        null,
        2
      );
      ...
      return JSON.stringify(
        command.verify
          ...
          : command.bundle
            ...
            : await createCompactionReport(context.cwd, command.runId),
        null,
        2
      );
    case "evidence-report":
      return JSON.stringify(
        await createEvidenceReport(context.cwd, {
          deep: command.deep,
          policy: command.policy
        }),
        null,
      ...
      return JSON.stringify(
        command.all
          ...
          : command.runId
            ...
            : { error: "Usage: /disposal-eligibility <run-id> | /disposal-eligibility --all" },
        null,
        2
      );
    ...
    case "disposal-plan":
      return JSON.stringify(
        await createDisposalPlan(context.cwd, command.runId, {
    ...
    case "disposal-attestation":
      ...
      return JSON.stringify(
        await createDisposalAttestation(context.cwd, command.runId, {
          sign: command.sign,
          confirm: command.confirm
        }),
        null,
      ...
      return JSON.stringify(
        command.add
          ...
          : command.verify
            ...
            : await getDisposalApprovals(context.cwd, command.runId),
        null,
        2
      );
    ...
    case "disposal-execute":
      ...
      return JSON.stringify(
        await executeDisposal(context.cwd, command.runId, { confirm: command.confirm }),
        null,
        2
      );
    case "disposal-report":
      return JSON.stringify(
        await createDisposalReport(context.cwd, { deep: command.deep }),
        null,
        2
      );
    case "dogfood":
      if (command.command === "plan") {
        return JSON.stringify(
          await createDogfoodPlan(context.cwd, { writeFixtures: command.writeFixtures }),
          null,
          2
        );
      }
      if (command.command === "fixtures") {
        ...
        return JSON.stringify(
          command.clean
            ...
            : await createDogfoodFixtures(context.cwd),
          null,
          2
        );
      }
      if (command.command === "run") {
        ...
        return JSON.stringify(
          await runDogfood(context.cwd, {
            fixture: command.fixture as never,
      ...
      return JSON.stringify(
        (await latestDogfoodReport(context.cwd)) ?? { error: "No report" },
        null,
        2
      );
    case "live-smoke":
    ...
    case "scanner-check":
      ...
      return JSON.stringify(
        await scannerReadiness(context.cwd, {
    ...
    case "beta-warnings":
      return JSON.stringify(
        command.command
          ...
          : await collectBetaWarnings(context.cwd),
        null,
        2
      );
    ...
    case "beta-rc":
      return JSON.stringify(
        await createBetaRcReport(context.cwd, {
          channel: command.channel as never,
          strict: command.strict
        }),
        null,
        2
    ...
    case "beta-trial":
      return JSON.stringify(
        command.command === "create"
          ? await createBetaTrialPack(context.cwd, command.target as never)
          : command.command === "list"
            ? await listBetaTrials(context.cwd)
            : await showBetaTrial(context.cwd, command.trialId ?? ""),
        null,
        2
      );
    case "doctor":
