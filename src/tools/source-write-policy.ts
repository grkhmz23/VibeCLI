...
export function validateSourcePath(path: string, options: SourceWritePolicyOptions): string {
...
function hasUnsafeEnvAssignment(content: string): boolean {
  const assignmentPattern =
    /\b(OPENAI_API_KEY|ANTHROPIC_API_KEY|OPENROUTER_API_KEY|JWT_SECRET|DATABASE_URL|API_KEY|SECRET|TOKEN|PASSWORD)\s*=\s*["']?([^\s"']+)/g;
  for (const match of content.matchAll(assignmentPattern)) {
    const name = match[1] ?? "";
    const value = match[2] ?? "";
    const safeReferences = new Set([name, `$${name}`, `\${${name}}`]);
    if (safeReferences.has(value)) continue;
